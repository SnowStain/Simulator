...
def launcher_build_dir(root: Path, target_name: str, configuration: str) -> Path:
...
def resolve_built_executable(run_project: Path, configuration: str, output_dir: Path | None = None) -> Path:
    project_name = run_project.stem
    if output_dir is not None:
        executable = output_dir / f"{project_name}.exe"
    else:
...
def resolve_target_framework(project_path: Path) -> str:
    try:
    ...
    match = re.search(r"<TargetFrameworks>\s*([^<\s;]+)", text)
    if match:
        return match.group(1)
...
def run_with_dotnet(
    ...
    if not no_build:
        ...
        if not build_ok:
            ...
            return False
    try:
        executable = resolve_built_executable(run_project, configuration, output_dir=output_dir)
