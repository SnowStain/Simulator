...
地形运动里“斜坡豁免”和“台阶跨越”必须分开处理。当前约定：只有地形碰撞面法线满足 `normal.Y >= cos(20°)` 时，才按可通行斜坡处理；比 `20°` 更陡的面不再因为连续高度采样而被当成普通坡面放行。

这样上坡仍然有必要的贴地和高度过渡豁免，但台阶边、墙面、陡坎不会借用斜坡逻辑绕开车型越障约束。全向轮仍按自身低台阶能力处理，英雄/哨兵/工程的高台阶通过逻辑仍走正向爬升机构约束。
