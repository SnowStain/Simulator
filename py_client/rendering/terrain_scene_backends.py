...
def _sample_terrain_scene_data(renderer, map_manager, map_rgb):
    ...
    data = {
        ...
        'world_center_xs': world_center_xs,
        'world_center_ys': world_center_ys,
...
def _terrain_surface_signature(game_engine):
    ...
    facets = []
    for facet in list(surface_config.get('facets') or []):
        ...
        heights = tuple(round(float(height), 3) for height in list(facet.get('heights_m') or []))
        facets.append((
            str(facet.get('id') or ''),
            str(facet.get('type') or ''),
            str(facet.get('team') or ''),
            str(facet.get('source') or ''),
            str(facet.get('top_color') or ''),
            str(facet.get('side_color') or ''),
            tuple(points),
            heights,
        ))
    return (tuple(primitives), tuple(facets))
...
def _point_in_polygon_2d(x, y, polygon):
    if len(polygon) < 3:
        return False
    inside = False
    ...
    for current_x, current_y in polygon:
        ...
        if intersects:
            ...
            cross_x = (previous_x - current_x) * (y - current_y) / denominator + current_x
            if x < cross_x:
                inside = not inside
...
def _terrain_surface_scene_facets(game_engine, data, map_rgb=None):
    ...
    center_offset_y = data['grid_height'] / 2.0
    vertical_scale = float(data.get('height_scene_scale', 1.0))
    fallback_side_color = surface_config.get('side_color') or '#4B4F55'
    facets = []
    for facet in list(surface_config.get('facets') or []):
        if not isinstance(facet, dict):
            continue
        ...
        if len(world_points) < 3:
            continue
        heights_m = [max(0.0, float(height)) for height in list(facet.get('heights_m') or [])]
        while len(heights_m) < len(world_points):
            heights_m.append(heights_m[-1] if heights_m else 0.0)

        center_world_x = sum(point[0] for point in world_points) / len(world_points)
        center_world_y = sum(point[1] for point in world_points) / len(world_points)
        top_color = _parse_hex_color(facet.get('top_color'), _sample_map_rgb_color(map_rgb, center_world_x, center_world_y))
        side_color = _parse_hex_color(facet.get('side_color') or fallback_side_color)
        scene_points = []
        for index, (world_x, world_y) in enumerate(world_points):
            scene_points.append((
                world_x / max(sampled_cell_size, 1e-6) - center_offset_x,
                heights_m[index] * vertical_scale,
                world_y / max(sampled_cell_size, 1e-6) - center_offset_y,
            ))
        facets.append({
            'id': str(facet.get('id') or ''),
            'type': str(facet.get('type') or 'slope'),
            'world_points': world_points,
            'scene_points': scene_points,
            'heights_m': heights_m[:len(world_points)],
            'top_color_rgb': top_color,
            'side_color_rgb': side_color,
            'top_color': tuple(channel / 255.0 for channel in top_color),
            'side_color': tuple(channel / 255.0 for channel in side_color),
...
def _terrain_surface_facet_cover_mask(data, facets):
    if not facets:
        return None
    world_center_xs = data.get('world_center_xs')
    world_center_ys = data.get('world_center_ys')
    if world_center_xs is None or world_center_ys is None:
        return None
    ...
    for facet in facets:
        polygon = facet.get('world_points') or []
        if len(polygon) < 3:
            continue
        ...
        candidate_xs = np.where((world_center_xs >= min_x) & (world_center_xs <= max_x))[0]
        candidate_ys = np.where((world_center_ys >= min_y) & (world_center_ys <= max_y))[0]
        if candidate_xs.size == 0 or candidate_ys.size == 0:
            continue
        for grid_y in candidate_ys:
            center_y = float(world_center_ys[grid_y])
            for grid_x in candidate_xs:
                if _point_in_polygon_2d(float(world_center_xs[grid_x]), center_y, polygon):
                    mask[grid_y, grid_x] = True
    return mask


def _append_surface_facet_vertices(vertices, facet):
    scene_points = facet.get('scene_points') or []
    if len(scene_points) < 3:
        return
    top_color = list(facet['top_color'])
    side_color = list(facet['side_color'])
    bottom_color = [channel * 0.55 for channel in side_color]

    anchor = scene_points[0]
    for index in range(1, len(scene_points) - 1):
        b = scene_points[index]
        c = scene_points[index + 1]
        vertices.extend((*anchor, *top_color, *b, *top_color, *c, *top_color))
        anchor_bottom = (anchor[0], 0.0, anchor[2])
        c_bottom = (c[0], 0.0, c[2])
        b_bottom = (b[0], 0.0, b[2])
        vertices.extend((*anchor_bottom, *bottom_color, *c_bottom, *bottom_color, *b_bottom, *bottom_color))

    for index in range(len(scene_points)):
        next_index = (index + 1) % len(scene_points)
        a = scene_points[index]
        b = scene_points[next_index]
        if max(a[1], b[1]) <= 1e-4:
            continue
        _append_face(
            vertices,
...
class SoftwareTerrainSceneBackend:
    ...
    def render_scene(self, renderer, game_engine, size, map_rgb=None):
        ...
        surface_facets = _terrain_surface_scene_facets(game_engine, data, map_rgb)
        facet_cover_mask = _terrain_surface_facet_cover_mask(data, surface_facets)
        ...
        def build_entries(tile_width):
            ...
            for grid_y in range(grid_height):
                for grid_x in range(grid_width):
                    if facet_cover_mask is not None and facet_cover_mask[grid_y, grid_x]:
                        continue
        ...
        for facet in surface_facets:
            ...
            if len(scene_points) < 3:
                continue
            projected_top = [project_point(point[0], point[2], point[1]) for point in scene_points]
            projected_bottom = [project_point(point[0], point[2], 0.0) for point in scene_points]
            top_color = facet['top_color_rgb']
            side_color = facet['side_color_rgb']
            bottom_color = tuple(max(0, min(255, int(channel * 0.58))) for channel in side_color)
            anchor_top = projected_top[0]
            anchor_bottom = projected_bottom[0]
            for index in range(1, len(projected_top) - 1):
                top_triangle = [anchor_top, projected_top[index], projected_top[index + 1]]
                bottom_triangle = [anchor_bottom, projected_bottom[index + 1], projected_bottom[index]]
                facet_faces.append((sum(point[2] for point in top_triangle) / 3.0, top_color, [(point[0], point[1]) for point in top_triangle]))
                facet_faces.append((sum(point[2] for point in bottom_triangle) / 3.0, bottom_color, [(point[0], point[1]) for point in bottom_triangle]))
            for index in range(len(projected_top)):
                next_index = (index + 1) % len(projected_top)
                a_top = projected_top[index]
                b_top = projected_top[next_index]
                a_bottom = projected_bottom[index]
                b_bottom = projected_bottom[next_index]
                if max(scene_points[index][1], scene_points[next_index][1]) <= 1e-4:
                    continue
                quad = [a_top, b_top, b_bottom, a_bottom]
                facet_faces.append((sum(point[2] for point in quad) / 4.0, side_color, [(point[0], point[1]) for point in quad]))
            facet_outlines.append([(point[0], point[1]) for point in projected_top])

        facet_faces.sort(key=lambda item: item[0])
        for _, face_color, face_points in facet_faces:
            pygame.draw.polygon(surface, face_color, face_points)
        for outline in facet_outlines:
            pygame.draw.polygon(surface, renderer.colors['panel_border'], outline, 1)
...
class ModernGLTerrainSceneBackend:
    ...
    def _ensure_geometry(self, renderer, map_manager, map_rgb):
        ...
        surface_facets = _terrain_surface_scene_facets(renderer.game_engine, data, map_rgb)
        facet_cover_mask = _terrain_surface_facet_cover_mask(data, surface_facets)
        vertices = []
        ...
        for facet in surface_facets:
            _append_surface_facet_vertices(vertices, facet)
        ...
        max_facet_height = 0.0
        if surface_facets:
            max_facet_height = max(max(point[1] for point in facet.get('scene_points') or [(0.0, 0.0, 0.0)]) for facet in surface_facets)

        self.scene_bounds = (
            max(1.0, float(grid_width)),
            max(1.0, float(grid_height)),
            max(1.0, float(np.max(sampled_heights) * vertical_scale + base_thickness), max_box_height + base_thickness, max_facet_height + base_thickness),
            int(scene_step),
        )
