...
class RendererSidebarMixin:
    ...
    def render_terrain_panel(self, game_engine, rect, selected_facility):
        ...
        if region.get('type') in {'base', 'outpost', 'energy_mechanism', 'dog_hole'}:
            ...
            if region.get('type') == 'dog_hole':
                model_specs = [
                    ('model_yaw_deg', '狗洞朝向', -180.0, 180.0),
                    ('model_bottom_offset_m', '底面离地', -1.0, 3.0),
                    ('model_clear_width_m', '净宽', 0.05, 3.0),
                    ('model_clear_height_m', '净高', 0.05, 2.0),
                    ('model_depth_m', '厚度', 0.03, 2.0),
                    ('model_frame_thickness_m', '侧梁厚', 0.01, 1.0),
                    ('model_top_beam_thickness_m', '上梁厚', 0.01, 1.0),
                ]
