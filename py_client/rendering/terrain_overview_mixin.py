...
class TerrainOverviewMixin:
    ...
    def _close_terrain_3d_window(self, exit_terrain_mode=False):
        self._sync_terrain_observer_mouse_capture(False)
    ...
    def _sync_terrain_observer_mouse_capture(self, active):
        desired = bool(active)
        if getattr(self, 'terrain_observer_mouse_captured', False) == desired:
            return
        self.terrain_observer_mouse_captured = desired
        try:
    ...
    def _terrain_scene_render_size(self, scene_rect, backend=None):
        ...
        if getattr(self, 'terrain_scene_camera_override', None) is not None:
            return (width, height)
        if getattr(self, 'edit_mode', None) != 'terrain' or getattr(self, 'terrain_view_mode', '3d') != '3d':
            return (width, height)
        backend_name = str(getattr(backend, 'name', '') or '').lower()
        render_scale = float(getattr(self, 'terrain_editor_render_scale', 1.0))
        if backend_name == 'software':
            render_scale = 1.0
        render_scale = max(1.0, min(2.0, render_scale))
        return (
            max(width, int(round(width * render_scale))),
            max(height, int(round(height * render_scale))),
        )
    ...
    def _render_terrain_scene_surface(self, game_engine, scene_rect, map_rgb):
        ...
        try:
            ...
            if cache_key is not None and getattr(self, 'terrain_scene_surface_key', None) == cache_key and cached_surface is not None:
                return cached_surface
            render_size = self._terrain_scene_render_size(scene_rect, backend=backend)
            surface = backend.render_scene(self, game_engine, render_size, map_rgb)
            if render_size != scene_rect.size:
                surface = pygame.transform.smoothscale(surface, scene_rect.size)
            if cache_key is not None:
    ...
    def _build_full_terrain_3d_surface(self, game_engine, size):
        ...
        max_scene_height = 520 if self._terrain_overview_embedded_mode() else 420
        scene_height = max(220, min(max_scene_height, int(available_vertical * 0.62)))
        ...
        scene_hint_text = '上半区左键按当前形状设置范围，2D 模式可直接框选。'
        if self.terrain_view_mode == '2d':
            scene_hint_text = '2D: 滚轮缩放，拖拽视口，左键直接编辑当前选区。'
        elif self.terrain_view_mode == '3d':
            if getattr(self, 'terrain_observer_mouse_captured', False):
                scene_hint_text = '3D观察者: 鼠标控视角 | WASD平移 | Q/E升降 | Shift加速 | ESC/G退出'
            else:
                scene_hint_text = '3D: G进入观察者 | WASD平移 | Q/E升降 | 滚轮缩放 | 右键拖动旋转'
        ...
        if self.terrain_view_mode == '3d':
            observer_button = pygame.Rect(width - 278, row2_y, 82, 28)
            ...
            observer_label = '释放视角' if getattr(self, 'terrain_observer_mouse_captured', False) else '观察者'
            self._draw_surface_button(surface, observer_button, observer_label, getattr(self, 'terrain_observer_mouse_captured', False))
            ...
            self.terrain_overview_ui['buttons'].extend([
                (observer_button, 'terrain_observer_toggle'),
    ...
    def _render_terrain_scene_2d_surface(self, game_engine, scene_rect):
        ...
        scene_cache = getattr(self, 'terrain_scene_2d_cache', None)
        if getattr(self, 'terrain_scene_2d_cache_key', None) != scene_cache_key or not isinstance(scene_cache, dict):
            scaler = pygame.transform.smoothscale if use_smooth else pygame.transform.scale
            base_surface = scaler(source.subsurface(source_rect), map_rect.size)
            ...
            facility_surface = scaler(facility_overlay.subsurface(facility_source_rect), map_rect.size)
            scene_cache = {
                'base_surface': base_surface,
                'facility_surface': facility_surface,
            }
            self.terrain_scene_2d_cache = scene_cache
            self.terrain_scene_2d_cache_key = scene_cache_key
        surface.blit(scene_cache['base_surface'], map_rect.topleft)
        ...
        surface.blit(scene_cache['facility_surface'], map_rect.topleft)
    ...
    def _handle_terrain_overview_event(self, event, game_engine):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_ESCAPE:
                if getattr(self, 'terrain_observer_mouse_captured', False):
                    self._sync_terrain_observer_mouse_capture(False)
                elif self.polygon_points or self.slope_region_points:
                    ...
                    self._close_terrain_3d_window(exit_terrain_mode=False)
                return True
            if self.terrain_view_mode == '3d' and event.key == pygame.K_g:
                self._sync_terrain_observer_mouse_capture(not bool(getattr(self, 'terrain_observer_mouse_captured', False)))
                self._invalidate_terrain_scene_cache()
                return True
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if getattr(self, 'terrain_observer_mouse_captured', False) and self.terrain_view_mode == '3d':
                rel = getattr(event, 'rel', (0, 0))
                if abs(rel[0]) + abs(rel[1]) > 0:
                    self.terrain_3d_camera_yaw += rel[0] * 0.010
                    self.terrain_3d_camera_pitch = max(0.18, min(1.15, self.terrain_3d_camera_pitch - rel[1] * 0.005))
                    self._invalidate_terrain_scene_cache()
                return True
