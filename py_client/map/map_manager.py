...
class MapManager:
    ...
    def _try_load_runtime_grid_bundle(self):
        ...
        self.raster_dirty = False
        self._clear_non_blocking_facility_movement_regions()
        return True

    def _clear_non_blocking_facility_movement_regions(self):
        if self.movement_block_map is None:
            return
        for region in self.facilities:
            if region.get('type') not in {'dog_hole', 'rugged_road', 'undulating_road'}:
                continue
            if bool(region.get('blocks_movement', False)):
                continue
            ...
            self.movement_block_map[rows, cols][mask] = False
        self.move_block_map = self.movement_block_map
    ...
    def _terrain_override_payload(self, grid_x, grid_y, terrain_type, height_m, team='neutral', blocks_movement=None, blocks_vision=None):
        return {
            ...
            'blocks_movement': bool(blocks_movement) if blocks_movement is not None else terrain_type in {'wall', 'boundary'},
            ...
            for grid_x, grid_y in self._iter_scaled_grid_cells(
                ...
                if str(cell.get('surface_mode', '')).strip():
                    ...
                    if isinstance(cell.get('plane_axis'), (list, tuple)) and len(cell.get('plane_axis')) >= 2:
                        normalized['plane_axis'] = [round(float(cell['plane_axis'][0]), 6), round(float(cell['plane_axis'][1]), 6)]
                    ...
                    surface_facet_id = str(cell.get('surface_facet_id') or '').strip()
                    if surface_facet_id:
                        normalized['surface_facet_id'] = surface_facet_id
                    surface_facet_source = str(cell.get('surface_facet_source') or '').strip()
                    if surface_facet_source:
                        normalized['surface_facet_source'] = surface_facet_source
                    surface_facet_type = str(cell.get('surface_facet_type') or '').strip()
                    if surface_facet_type:
                        normalized['surface_facet_type'] = surface_facet_type
                    surface_facet_team = str(cell.get('surface_facet_team') or '').strip()
                    if surface_facet_team:
                        normalized['surface_facet_team'] = surface_facet_team
                    if isinstance(cell.get('surface_facet_points'), (list, tuple)):
                        surface_facet_points = []
                        for point in cell.get('surface_facet_points') or []:
                            if not isinstance(point, (list, tuple)) or len(point) < 2:
                                continue
                            surface_facet_points.append([round(float(point[0]), 3), round(float(point[1]), 3)])
                        if len(surface_facet_points) >= 3:
                            normalized['surface_facet_points'] = surface_facet_points
                    if isinstance(cell.get('surface_facet_heights_m'), (list, tuple)):
                        surface_facet_heights = [round(float(height), 3) for height in cell.get('surface_facet_heights_m') or []]
                        if surface_facet_heights:
                            normalized['surface_facet_heights_m'] = surface_facet_heights
                    surface_facet_top_color = str(cell.get('surface_facet_top_color') or '').strip()
                    if surface_facet_top_color:
                        normalized['surface_facet_top_color'] = surface_facet_top_color
                    surface_facet_side_color = str(cell.get('surface_facet_side_color') or '').strip()
                    if surface_facet_side_color:
                        normalized['surface_facet_side_color'] = surface_facet_side_color
    ...
    def _build_slope_plane_surface_payload(self, points, slope_info, terrain_type, team='neutral'):
        normalized_points = self._normalize_points(points)
        if len(normalized_points) < 3 or not slope_info.get('changed'):
            return {}

        min_height = float(slope_info.get('min_height', 0.0))
        max_height = float(slope_info.get('max_height', min_height))
        low_center_x, low_center_y = slope_info.get('low_point', (0.0, 0.0))
        high_center_x, high_center_y = slope_info.get('high_point', (low_center_x, low_center_y))
        ...
        for point_x, point_y in normalized_points:
            ...
            if axis_length_sq <= 1e-6:
                ...
                blend = ((point_x - float(low_center_x)) * axis_x + (point_y - float(low_center_y)) * axis_y) / axis_length_sq
                blend = max(0.0, min(1.0, blend))
            facet_points.append([round(point_x, 3), round(point_y, 3)])
            facet_heights.append(round(min_height + (max_height - min_height) * blend, 3))

        facet_crc_payload = json.dumps(
            {
                'points': facet_points,
                'heights_m': facet_heights,
                'terrain_type': str(terrain_type or 'flat'),
                'team': str(team or 'neutral'),
            },
            ensure_ascii=False,
            sort_keys=True,
            separators=(',', ':'),
        ).encode('utf-8')
        surface_config = dict(self.config.get('map', {}).get('terrain_surface', {}) or {})
        return {
            ...
            'surface_facet_id': f"slope_plane_{zlib.crc32(facet_crc_payload) & 0xffffffff:08x}",
            'surface_facet_source': 'terrain_grid_slope_plane',
            'surface_facet_type': 'slope',
            'surface_facet_team': str(team or 'neutral'),
            'surface_facet_points': facet_points,
            'surface_facet_heights_m': facet_heights,
            'surface_facet_top_color': str(surface_config.get('default_facet_top_color') or '#8A9576'),
            'surface_facet_side_color': str(surface_config.get('side_color') or '#4B4F55'),
        }

    def _normalize_terrain_surface_facet(self, facet, fallback_side_color='#4B4F55'):
        if not isinstance(facet, dict):
            return None

        points = []
        for point in facet.get('points') or []:
            if not isinstance(point, (list, tuple)) or len(point) < 2:
                continue
        ...
        heights = [round(float(height), 3) for height in facet.get('heights_m') or []]
        while len(heights) < len(points):
            heights.append(heights[-1] if heights else 0.0)

        facet_id = str(facet.get('id') or '').strip()
        if not facet_id:
            facet_crc_payload = json.dumps(
                {'points': points, 'heights_m': heights},
                ensure_ascii=False,
                sort_keys=True,
                separators=(',', ':'),
            ).encode('utf-8')
            facet_id = f"facet_{zlib.crc32(facet_crc_payload) & 0xffffffff:08x}"

        normalized = {
            'id': facet_id,
            'type': str(facet.get('type') or 'slope'),
            'team': str(facet.get('team') or 'neutral'),
            'top_color': str(facet.get('top_color') or '#8A9576'),
            'side_color': str(facet.get('side_color') or fallback_side_color or '#4B4F55'),
            'points': points,
            'heights_m': heights[:len(points)],
        }
        source = str(facet.get('source') or '').strip()
        if source:
            normalized['source'] = source
        return normalized

    def _collect_terrain_surface_facets(self):
        grouped = {}
        fallback_side_color = str(self.config.get('map', {}).get('terrain_surface', {}).get('side_color') or '#4B4F55')
        for cell in self.terrain_grid_overrides.values():
            if str(cell.get('surface_mode') or '').strip() != 'slope_plane':
                continue
            facet_id = str(cell.get('surface_facet_id') or '').strip()
            if not facet_id:
                continue
            current = grouped.get(facet_id)
            if current is None or len(current.get('surface_facet_points') or []) < 3:
                grouped[facet_id] = dict(cell)

        facets = []
        for facet_id in sorted(grouped.keys()):
            cell = grouped[facet_id]
            facet = self._normalize_terrain_surface_facet(
                {
                    'id': facet_id,
                    'source': str(cell.get('surface_facet_source') or 'terrain_grid_slope_plane'),
                    'type': str(cell.get('surface_facet_type') or 'slope'),
                    'team': str(cell.get('surface_facet_team') or cell.get('team') or 'neutral'),
                    'top_color': str(cell.get('surface_facet_top_color') or '#8A9576'),
                    'side_color': str(cell.get('surface_facet_side_color') or fallback_side_color),
                    'points': cell.get('surface_facet_points') or [],
                    'heights_m': cell.get('surface_facet_heights_m') or [],
                },
                fallback_side_color=fallback_side_color,
            )
    ...
    def export_terrain_surface_config(self, preset_name=None):
        with self._edit_state_lock:
            ...
            existing = deepcopy(self.config.get('map', {}).get('terrain_surface', {}) or {})
            ...
            fallback_side_color = str(existing.get('side_color') or '#4B4F55')
            preserved_facets = []
            for facet in list(existing.get('facets') or []):
                normalized = self._normalize_terrain_surface_facet(facet, fallback_side_color=fallback_side_color)
                if normalized is None or str(normalized.get('source') or '').strip() == 'terrain_grid_slope_plane':
                    continue
                preserved_facets.append(normalized)

            surface_config = dict(existing)
            surface_config['map_type'] = str(existing.get('map_type') or 'terrain_surface_map')
            surface_config['descriptor_path'] = descriptor_path
            surface_config['storage_kind'] = str(existing.get('storage_kind') or 'runtime_triangle_grid')
            surface_config['topology'] = str(existing.get('topology') or 'triangle_grid')
            surface_config['merge_mode'] = str(existing.get('merge_mode') or 'merged_exposed_faces')
            surface_config['split_mode'] = str(existing.get('split_mode') or 'diag_forward')
            surface_config['render_profile'] = str(existing.get('render_profile') or 'top_png_orthographic_side_solid')
            surface_config['top_face_mode'] = str(existing.get('top_face_mode') or 'orthographic_png')
            surface_config['side_face_mode'] = str(existing.get('side_face_mode') or 'solid_color')
            surface_config['side_color'] = fallback_side_color
            surface_config['top_normal_threshold'] = float(existing.get('top_normal_threshold', 0.9))
            surface_config['side_normal_threshold'] = float(existing.get('side_normal_threshold', 0.1))
            surface_config['base_color_image_path'] = base_color_image_path
            surface_config['facets'] = preserved_facets + self._collect_terrain_surface_facets()
    ...
    def _build_terrain_surface_descriptor(self, preset_name, descriptor_path, channel_files):
        ...
        return {
            ...
            'facets': deepcopy(surface_config.get('facets') or []),
        }
    ...
    def persist_runtime_grid_bundle(self, preset_name, preset_path=None):
        ...
        fd, temp_descriptor_path = tempfile.mkstemp(
            prefix=f'.{os.path.basename(descriptor_path)}.',
            suffix='.tmp',
            dir=directory,
            text=True,
        )
        try:
            ...
            os.replace(temp_descriptor_path, descriptor_path)
        except Exception:
            try:
                os.unlink(temp_descriptor_path)
            except OSError:
                pass
            raise
        self.runtime_grid_bundle = {
    ...
    def paint_terrain_plane_polygon(self, points, terrain_type, team='neutral', blocks_movement=None, blocks_vision=None, direction_start=None, direction_end=None):
        ...
        axis_y = float(high_center_y) - float(low_center_y)
        axis_length_sq = axis_x * axis_x + axis_y * axis_y
        surface_payload = self._build_slope_plane_surface_payload(
            normalized_points,
            slope_info,
            terrain_type,
            team=team,
    ...
    def _apply_region_to_raster(self, region):
        ...
        if replace_mask.any():
            ...
            elif region.get('type') == 'boundary':
                move_block = True
            elif region.get('type') in {'dog_hole', 'rugged_road', 'undulating_road'}:
                move_block = bool(region.get('blocks_movement', False))
    ...
    def _normalize_region(self, region):
        ...
        if normalized.get('type') == 'wall' or normalized.get('shape') == 'line':
            ...
            if normalized.get('type') == 'dog_hole':
                ...
                normalized.setdefault('model_yaw_deg', default_yaw)
                normalized.setdefault('model_bottom_offset_m', default_bottom)
                normalized.setdefault('model_clear_width_m', 0.8)
                normalized.setdefault('model_clear_height_m', 0.25)
                normalized.setdefault('model_depth_m', 0.25)
                normalized.setdefault('model_frame_thickness_m', 0.065)
                normalized.setdefault('model_top_beam_thickness_m', default_top_beam)
        return normalized
