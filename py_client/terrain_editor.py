...
class TerrainEditorEngine:
    def __init__(self, config_path='config.json', settings_path=None):
        ...
        self._save_idle = threading.Condition(self._save_lock)
        self._save_event = threading.Event()
        self._save_stop_event = threading.Event()
        self._pending_save_job = None
        self._active_save_jobs = 0
    ...
    def shutdown(self):
        ...
        asset_worker = getattr(self, '_asset_export_thread', None)
        if asset_worker is not None and asset_worker.is_alive():
            asset_worker.join(timeout=10.0)
    ...
    def _queue_save_job(self, job):
        with self._save_lock:
            ...
            self._save_idle.notify_all()
        self._save_event.set()

    def _claim_save_job(self):
        with self._save_lock:
            job = self._pending_save_job
            self._pending_save_job = None
            if job is not None:
                self._active_save_jobs += 1
            return job

    def _finish_save_job(self):
        with self._save_lock:
            self._active_save_jobs = max(0, self._active_save_jobs - 1)
            self._save_idle.notify_all()

    def _run_save_job(self, job):
        try:
            self._perform_save_job(job)
        finally:
            self._finish_save_job()

    def _drain_save_queue(self, timeout=15.0):
        deadline = time.monotonic() + max(0.5, timeout)
        while True:
            job = self._claim_save_job()
            if job is not None:
                self._run_save_job(job)
                continue
            with self._save_lock:
                if self._active_save_jobs <= 0 and self._pending_save_job is None:
                    return True
                remaining_sec = deadline - time.monotonic()
                if remaining_sec <= 0:
                    return False
                self._save_idle.wait(min(0.2, remaining_sec))
