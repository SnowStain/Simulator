...
public sealed class ArenaInteractionService
{
    ...
    private const double TerrainHighlandDamageTakenMult = 0.50;
        ...
        ICollection<FacilityInteractionEvent> events)
    {
        ...
        entity.DestroyedTimeSec = world.GameTimeSec;
        ...
        ArmorPlateTarget? firstHitPlate = null)
    {
        ...
        {
            bool large = teamState.EnergyTestForceLarge || gameTimeSec >= 180.0;
            StartEnergyAttempt(teamState, large, ResolveLargeEnergyAttemptSlot(gameTimeSec), gameTimeSec);
        }

        if (firstHitPlate.HasValue
            && SimulationCombatMath.TryParseEnergyArmIndex(firstHitPlate.Value.Id, out string plateTeam, out int armIndex)
            && string.Equals(plateTeam, team, StringComparison.OrdinalIgnoreCase)
        ...
        int ringScore)
    {
        ...
        if (CountActivatedEnergyDisks(teamState) >= 5)
        {
            if (teamState.EnergyLargeMechanismActive)
            {
                ...
                AwardTeamEnergyActivationExperience(world, shooter.Team, 750.0);
            }
            else
            {
                teamState.EnergySmallChanceUsed = true;
                GrantSmallEnergyBuff(teamState);
                AwardTeamEnergyActivationExperience(world, shooter.Team, 1200.0);
            }
    ...
    {
        SimulationEntity[] receivers = world.Entities
            .Where(entity => entity.IsAlive
                && IsExperienceRobot(entity)
        ...
        double perRobot = Math.Max(0.0, totalExperience) / receivers.Length;
        foreach (SimulationEntity receiver in receivers)
        {
            GrantRobotExperience(receiver, perRobot);
        }
    }
    ...
    {
        if (appliedDamage <= 1e-6 || !IsExperienceRobot(shooter))
        {
            return;
        }

        double experience = ResolveDamageExperience(shooter, target, appliedDamage);
        GrantRobotExperience(shooter, experience);
    }

    internal static void GrantKillExperience(SimulationEntity shooter, SimulationEntity target)
    {
        if (!IsExperienceRobot(shooter))
        {
            return;
        }

        int targetLevel = Math.Clamp(target.Level <= 0 ? 1 : target.Level, 1, 10);
        int shooterLevel = Math.Clamp(shooter.Level <= 0 ? 1 : shooter.Level, 1, 10);
        double experience = 50.0 * targetLevel;
        if (shooterLevel >= targetLevel)
        {
            experience *= 1.0 + 0.2 * (targetLevel - shooterLevel);
        }

        GrantRobotExperience(shooter, Math.Max(0.0, experience));
    }

    internal static void GrantRobotExperience(SimulationEntity entity, double experience)
    {
        if (!IsExperienceRobot(entity) || experience <= 1e-6)
        {
            return;
        }

        entity.PendingExperienceDisplay += Math.Max(0.0, experience);
        entity.Experience = Math.Max(0.0, entity.Experience + experience);
        int maxLevel = Math.Clamp(entity.MaxLevel <= 0 ? 10 : entity.MaxLevel, 1, 10);
        int startLevel = entity.Level;
        while (entity.Level < maxLevel
            && entity.Experience >= ResolveRobotLevelExperienceThreshold(entity.Level + 1))
        {
            entity.Level++;
        }

        if (entity.Level > startLevel)
        {
            entity.PendingLevelUpCount += entity.Level - startLevel;
        }
    }

    private static double ResolveRobotLevelExperienceThreshold(int level)
    ...
    private static double ResolveDamageExperience(
        SimulationEntity shooter,
        SimulationEntity target,
        double appliedDamage)
    {
        if (appliedDamage <= 1e-6)
        {
            return 0.0;
        }
    ...
    private static bool TryConsumeEnergyOpportunityToken(SimulationTeamState teamState, double gameTimeSec, out bool large)
    {
        ...
        if (teamState.EnergyTestAlwaysAvailable)
        {
            large = teamState.EnergyTestForceLarge || gameTimeSec >= 180.0;
            return true;
        }
