...
public sealed class SimulationBootstrapService
{
    ...
    {
        AnnotatedEnergyMechanismDefinition? annotatedEnergyDefinition = EnergyMechanismAnnotationLoader.TryLoad(
            mapPreset,
            ResolveMetersPerWorldUnit(config, mapPreset));
        AnnotatedOutpostDefinition? annotatedOutpostDefinition = OutpostAnnotationLoader.TryLoad(
            mapPreset,
            ResolveMetersPerWorldUnit(config, mapPreset));
        ...
        InitializeRuntimeModelTransforms(world, mapPreset);
        ...
        AddStructureEntities(world, mapPreset, annotatedEnergyDefinition, annotatedOutpostDefinition);
        AddAnnotatedEnergyMechanismEntity(world, annotatedEnergyDefinition);
        AddAnnotatedOutpostEntities(world, annotatedOutpostDefinition);
        return world;
    }

    private static void AddStructureEntities(
        SimulationWorldState world,
        MapPresetDefinition mapPreset,
        AnnotatedEnergyMechanismDefinition? annotatedEnergyDefinition,
        AnnotatedOutpostDefinition? annotatedOutpostDefinition)
    {
        int energyIndex = 0;
        foreach (FacilityRegion region in mapPreset.Facilities)
        {
            ...
            if (string.Equals(type, "energy_mechanism", StringComparison.OrdinalIgnoreCase)
                && annotatedEnergyDefinition is not null)
            {
                continue;
            }

            if (string.Equals(type, "outpost", StringComparison.OrdinalIgnoreCase)
                && annotatedOutpostDefinition is not null
                && annotatedOutpostDefinition.Teams.ContainsKey(region.Team))
            {
                continue;
            }
    ...
    private static void AddAnnotatedEnergyMechanismEntity(
        SimulationWorldState world,
        AnnotatedEnergyMechanismDefinition? definition)
    {
        ...
        IReadOnlyList<SimulationEntity> existingEnergyMechanisms = world.Entities
            .Where(entity => string.Equals(entity.EntityType, "energy_mechanism", StringComparison.OrdinalIgnoreCase))
            .ToArray();
        if (existingEnergyMechanisms.Count > 0)
        {
            foreach (SimulationEntity entity in existingEnergyMechanisms)
            {
                entity.AnnotatedEnergyMechanism = definition;
                entity.X = definition.CenterWorldX;
                entity.Y = definition.CenterWorldY;
                double annotatedCollisionRadiusM = Math.Max(0.60, definition.MaxRadiusM);
                entity.BodyHeightM = Math.Max(entity.BodyHeightM, definition.ApproximateHeightM);
                entity.BodyWidthM = Math.Max(entity.BodyWidthM, annotatedCollisionRadiusM * 2.0);
                entity.BodyLengthM = Math.Max(entity.BodyLengthM, annotatedCollisionRadiusM * 2.0);
                entity.CollisionRadiusWorld = Math.Max(
                    entity.CollisionRadiusWorld,
                    annotatedCollisionRadiusM / Math.Max(world.MetersPerWorldUnit, 1e-6));
                entity.RuntimeModelToSceneMatrix = world.RuntimeModelToSceneMatrix;
                entity.RuntimeModelToWorldMatrix = world.RuntimeModelToWorldMatrix;
            }

            return;
        }

        double collisionRadiusM = Math.Max(0.60, definition.MaxRadiusM);
        world.Entities.Add(new SimulationEntity
        {
            Id = "annotated_energy_mechanism",
            Team = "neutral",
            EntityType = "energy_mechanism",
            RoleKey = "energy_mechanism",
            X = definition.CenterWorldX,
            Y = definition.CenterWorldY,
            AngleDeg = 0.0,
            TurretYawDeg = 0.0,
            MaxHealth = 0.0,
            Health = 0.0,
            ...
            Ammo17Mm = 0,
            Ammo42Mm = 0,
            BodyHeightM = Math.Max(1.4, definition.ApproximateHeightM),
            BodyWidthM = collisionRadiusM * 2.0,
            BodyLengthM = collisionRadiusM * 2.0,
            BodyRenderWidthScale = 1.0,
            CollisionRadiusWorld = collisionRadiusM / Math.Max(world.MetersPerWorldUnit, 1e-6),
            AnnotatedEnergyMechanism = definition,
            RuntimeModelToSceneMatrix = world.RuntimeModelToSceneMatrix,
            RuntimeModelToWorldMatrix = world.RuntimeModelToWorldMatrix,
        });
    }

    private static void AddAnnotatedOutpostEntities(
        SimulationWorldState world,
        AnnotatedOutpostDefinition? definition)
    {
        if (definition is null)
        {
            return;
        ...
        foreach (SimulationEntity entity in world.Entities)
        {
            ...
            if (!definition.Teams.TryGetValue(entity.Team, out AnnotatedOutpostTeamDefinition? teamDefinition))
            {
                continue;
            }

            entity.AnnotatedOutpost = teamDefinition;
            entity.X = teamDefinition.PivotWorldX;
            entity.Y = teamDefinition.PivotWorldY;
            entity.RuntimeModelToSceneMatrix = world.RuntimeModelToSceneMatrix;
            entity.RuntimeModelToWorldMatrix = world.RuntimeModelToWorldMatrix;
        }

        foreach ((string team, AnnotatedOutpostTeamDefinition teamDefinition) in definition.Teams)
        {
            bool exists = world.Entities.Any(entity =>
            ...
            world.Entities.Add(new SimulationEntity
            {
                ...
                EntityType = "outpost",
                RoleKey = "outpost",
                X = teamDefinition.PivotWorldX,
                Y = teamDefinition.PivotWorldY,
                AngleDeg = ResolveStructureYawDeg(team, "outpost"),
                TurretYawDeg = ResolveStructureYawDeg(team, "outpost"),
                MaxHealth = 1500.0,
                Health = 1500.0,
                MaxPower = 0.0,
                Power = 0.0,
                ...
                BodyLengthM = 0.65,
                BodyRenderWidthScale = 1.0,
                CollisionRadiusWorld = 0.42 / Math.Max(world.MetersPerWorldUnit, 1e-6),
                AnnotatedOutpost = teamDefinition,
                RuntimeModelToSceneMatrix = world.RuntimeModelToSceneMatrix,
                RuntimeModelToWorldMatrix = world.RuntimeModelToWorldMatrix,
            });
        }
    ...
    private static void InitializeRuntimeModelTransforms(SimulationWorldState world, MapPresetDefinition mapPreset)
    {
        Matrix4x4 modelToScene = Matrix4x4.Identity;
        Matrix4x4 modelToWorld = Matrix4x4.Identity;
        if (FineTerrainRuntimeSceneScaleResolver.TryResolve(mapPreset, out FineTerrainRuntimeSceneScale scale))
        {
            modelToScene = FineTerrainAnnotationWorldSpace.CreateModelToSceneMatrix(
                mapPreset.FieldLengthM,
                mapPreset.FieldWidthM,
                scale.ModelCenter.X,
                scale.ModelCenter.Y,
                scale.ModelCenter.Z,
                scale.XMetersPerModelUnit,
                scale.YMetersPerModelUnit,
                scale.ZMetersPerModelUnit);
            modelToWorld = FineTerrainAnnotationWorldSpace.CreateModelToWorldMatrix(
                mapPreset.FieldLengthM,
                mapPreset.FieldWidthM,
        ...
        world.RuntimeModelToSceneMatrix = modelToScene;
        world.RuntimeModelToWorldMatrix = modelToWorld;
        world.RuntimeModelTransforms["model_to_scene_metric"] = modelToScene;
        world.RuntimeModelTransforms["model_to_world_units"] = modelToWorld;
    }
    ...
    {
        if (ShouldUseIntrinsicMapScale(mapPreset))
        {
    ...
    private static bool ShouldUseIntrinsicMapScale(MapPresetDefinition mapPreset)
    {
        return string.IsNullOrWhiteSpace(mapPreset.SourcePath)
