...
public sealed class RobotAppearanceProfileDefinition
{
    ...
    [JsonPropertyName("custom_primitives")]
    [Browsable(false)]
    public List<RobotAppearanceCustomPrimitiveDefinition> CustomPrimitives { get; set; } = new();

    [JsonPropertyName("custom_anchors")]
    [Browsable(false)]
    public List<RobotAppearanceAnchorDefinition> CustomAnchors { get; set; } = new();

    [JsonPropertyName("custom_links")]
    [Browsable(false)]
    public List<RobotAppearanceLinkDefinition> CustomLinks { get; set; } = new();
    ...
    public void EnsureInitialized()
    {
        ...
        CustomPrimitives ??= new List<RobotAppearanceCustomPrimitiveDefinition>();
        CustomAnchors ??= new List<RobotAppearanceAnchorDefinition>();
        CustomLinks ??= new List<RobotAppearanceLinkDefinition>();
        ...
        {
            primitive.EnsureInitialized();
        }

        foreach (RobotAppearanceAnchorDefinition anchor in CustomAnchors)
        {
            anchor.EnsureInitialized();
        }

        foreach (RobotAppearanceLinkDefinition link in CustomLinks)
        {
...
public sealed class RobotAppearanceCustomPrimitiveDefinition
{
    [JsonPropertyName("id")]
    ...
    [JsonPropertyName("size_m")]
    public List<double> SizeM { get; set; } = new() { 0.06, 0.04, 0.04 };

    [JsonPropertyName("offset_m")]
    public List<double> OffsetM { get; set; } = new() { 0.0, 0.0, 0.0 };

    [JsonPropertyName("rotation_ypr_deg")]
    public List<double> RotationYprDeg { get; set; } = new() { 0.0, 0.0, 0.0 };

    [JsonPropertyName("color_rgb")]
    public List<int> ColorRgb { get; set; } = new() { 188, 192, 198 };

    public void EnsureInitialized()
    {
        ParentPart = NormalizeText(ParentPart, "body");
        ComponentScope = NormalizeText(ComponentScope, "single");
        PrimitiveType = NormalizeText(PrimitiveType, "box");
        ComponentIndex = Math.Max(0, ComponentIndex);
        SizeM = RobotAppearanceCustomModelHelpers.NormalizeVector(SizeM, 0.06, 0.04, 0.04, 0.002);
        OffsetM = RobotAppearanceCustomModelHelpers.NormalizeVector(OffsetM, 0.0, 0.0, 0.0, double.MinValue);
        RotationYprDeg = RobotAppearanceCustomModelHelpers.NormalizeVector(RotationYprDeg, 0.0, 0.0, 0.0, double.MinValue);
        ColorRgb = RobotAppearanceCustomModelHelpers.NormalizeRgb(ColorRgb, 188, 192, 198);
        if (string.IsNullOrWhiteSpace(Id))
        {
            Id = $"primitive_{Guid.NewGuid():N}";
        }
    }
...
public sealed class RobotAppearanceAnchorDefinition
{
    [JsonPropertyName("id")]
    public string Id { get; set; } = string.Empty;
    ...
    public void EnsureInitialized()
    {
        ...
        if (string.IsNullOrWhiteSpace(Id))
        {
            Id = $"anchor_{Guid.NewGuid():N}";
        }
    }
}

public sealed class RobotAppearanceLinkDefinition
{
    [JsonPropertyName("id")]
    public string Id { get; set; } = string.Empty;
    ...
    [JsonPropertyName("start_anchor_id")]
    public string StartAnchorId { get; set; } = string.Empty;

    [JsonPropertyName("end_anchor_id")]
    public string EndAnchorId { get; set; } = string.Empty;
...
internal static class RobotAppearanceCustomModelHelpers
{
    public static List<double> NormalizeVector(List<double>? values, double x, double y, double z, double minValue)
    {
        var normalized = values is { Count: >= 3 }
            ? new List<double> { values[0], values[1], values[2] }
            : new List<double> { x, y, z };

        if (!double.IsFinite(normalized[0]))
        {
            normalized[0] = x;
        }

        if (!double.IsFinite(normalized[1]))
        {
            normalized[1] = y;
        }

        if (!double.IsFinite(normalized[2]))
        {
            normalized[2] = z;
        }
        ...
        {
            normalized[0] = Math.Max(minValue, normalized[0]);
            normalized[1] = Math.Max(minValue, normalized[1]);
            normalized[2] = Math.Max(minValue, normalized[2]);
        }

        return normalized;
    }

    public static List<int> NormalizeRgb(List<int>? values, int r, int g, int b)
    {
        if (values is { Count: >= 3 })
        {
            return new List<int>
