...
internal sealed class TerrainEditorForm : Form
{
    ...
    public TerrainEditorForm()
    {
        ...
        _editPreview.MapSelectionChanged += (_, _) => UpdateSelectionStatusEx();
    ...
    private Control BuildEditorPanel()
    {
        ...
        toolbar.Controls.Add(CreateButton("框选整体设施", (_, _) => AddFacilityGroupFromSelection(), 124));
    ...
    private void AddFacilityGroupFromSelection()
    {
        if (_document is null)
        {
            return;
        ...
        if (!TryResolveFacilityGroupBounds(out RectangleF selection))
        {
            _statusLabel.Text = "请先在顶视图中框选区域，或点选一个设施/斜面组件，再创建整体设施。";
            return;
        }
        ...
        var facility = new FacilityRegionEditorModel
        {
            Id = $"facility_cluster_{nextIndex}",
            ...
            HeightM = 0.08,
            Thickness = 10.0,
            GroupAsFacility = true,
            GroupSource = "selection_box",
        };

        FacilitySelectionGroupResult result = PopulateFacilityGroupSelection(facility, selection);
        _document.Facilities.Add(facility);
        RebindFacilityList();
        _facilityList.SelectedItem = facility;
        _tabControl.SelectedIndex = 2;
    ...
    private bool TryResolveFacilityGroupBounds(out RectangleF selection)
    {
        selection = RectangleF.Empty;
        if (_editPreview.MapSelection is RectangleF boxSelection
            && boxSelection.Width >= 2f
            && boxSelection.Height >= 2f)
        {
            selection = boxSelection;
            return true;
        }

        if (_facilityList.SelectedItem is FacilityRegionEditorModel facility
            && TryResolveFacilityBounds(facility, out selection))
        {
            return true;
        }

        if (_facetList.SelectedItem is TerrainFacetEditorModel facet
            && TryResolvePointBounds(facet.ParsePoints(), out selection))
        {
            return true;
        }
    ...
    private static bool TryResolveFacilityBounds(FacilityRegionEditorModel facility, out RectangleF bounds)
    {
        bounds = RectangleF.Empty;
        if (string.Equals(facility.Shape, "polygon", StringComparison.OrdinalIgnoreCase)
            && TryResolvePointBounds(facility.ParsePoints(), out bounds))
        {
            return true;
        }
        ...
        {
            float inflate = (float)Math.Max(1.0, facility.Thickness * 0.5);
            left -= inflate;
            right += inflate;
            top -= inflate;
            bottom += inflate;
    ...
    private static bool TryResolvePointBounds(IReadOnlyList<Simulator.Core.Map.Point2D> points, out RectangleF bounds)
    {
        ...
        float left = (float)points.Min(point => point.X);
        float top = (float)points.Min(point => point.Y);
        float right = (float)points.Max(point => point.X);
        float bottom = (float)points.Max(point => point.Y);
        bounds = RectangleF.FromLTRB(left, top, right, bottom);
        return bounds.Width >= 0.01f || bounds.Height >= 0.01f;
    }
    ...
    private void UpdateSelectionStatusEx()
    {
        if (_editPreview.MapSelection is not RectangleF selection || selection.Width < 2f || selection.Height < 2f)
        {
            return;
        ...
        _statusLabel.Text = string.Create(
            CultureInfo.InvariantCulture,
            $"已框选: x={selection.Left:0.#}..{selection.Right:0.#}, y={selection.Top:0.#}..{selection.Bottom:0.#}。可创建斜面或整体设施。");
    }

    private FacilitySelectionGroupResult PopulateFacilityGroupSelection(FacilityRegionEditorModel facility, RectangleF selection)
    {
        FineTerrainAnnotationDocument? annotation = TryLoadActiveFineTerrainAnnotation();
        if (annotation is null || _document is null)
        {
            return new FacilitySelectionGroupResult(0, 0, false);
        }

        var selectedComponentIds = new HashSet<int>();
        var selectedCompositeIds = new HashSet<int>();

        foreach (FineTerrainCompositeAnnotation composite in annotation.Composites)
        {
            if (!TryResolveCompositeSelectionBounds(annotation, composite, out RectangleF compositeBounds)
                || !selection.IntersectsWith(compositeBounds))
            {
                continue;
            }

            selectedCompositeIds.Add(composite.Id);
            foreach (int componentId in composite.ComponentIds)
            {
        ...
        {
            if (TryResolveComponentSelectionBounds(annotation, component, out RectangleF componentBounds)
                && selection.IntersectsWith(componentBounds))
            {
                selectedComponentIds.Add(component.Id);
            }
        }

        facility.GroupSource = "selection_box+annotation";
        facility.GroupCompositeIdsText = string.Join(", ", selectedCompositeIds.OrderBy(id => id));
        facility.GroupComponentIdsText = string.Join(", ", selectedComponentIds.OrderBy(id => id));
        return new FacilitySelectionGroupResult(selectedCompositeIds.Count, selectedComponentIds.Count, true);
    }

    private FineTerrainAnnotationDocument? TryLoadActiveFineTerrainAnnotation()
    {
        if (_document is null)
        {
            return null;
        ...
        return string.IsNullOrWhiteSpace(annotationPath)
            ? null
            : FineTerrainAnnotationDocument.TryLoad(annotationPath);
    }

    private bool TryResolveCompositeSelectionBounds(
        FineTerrainAnnotationDocument annotation,
        FineTerrainCompositeAnnotation composite,
        out RectangleF bounds)
    {
        bounds = RectangleF.Empty;
        var componentRects = new List<RectangleF>();
        foreach (int componentId in composite.ComponentIds)
        {
            if (annotation.ComponentsById.TryGetValue(componentId, out FineTerrainComponentAnnotation? component)
                && TryResolveComponentSelectionBounds(annotation, component, out RectangleF componentRect))
            {
                componentRects.Add(componentRect);
            }
        }
        ...
        if (componentRects.Count > 0)
        {
            float left = componentRects.Min(item => item.Left);
            float top = componentRects.Min(item => item.Top);
            float right = componentRects.Max(item => item.Right);
            float bottom = componentRects.Max(item => item.Bottom);
            bounds = RectangleF.FromLTRB(left, top, right, bottom);
            return true;
        }

        Vector3 positionModel = composite.PositionModel.ToVector3();
        PointF projected = ProjectModelPointToMap(annotation, positionModel.X, positionModel.Z);
        bounds = RectangleF.FromLTRB(projected.X - 2f, projected.Y - 2f, projected.X + 2f, projected.Y + 2f);
        return true;
    }

    private bool TryResolveComponentSelectionBounds(
        FineTerrainAnnotationDocument annotation,
        ...
        out RectangleF bounds)
    {
        ...
        PointF minPoint = ProjectModelPointToMap(annotation, component.Bounds.Min[0], component.Bounds.Min[2]);
        PointF maxPoint = ProjectModelPointToMap(annotation, component.Bounds.Max[0], component.Bounds.Max[2]);
    ...
    private PointF ProjectModelPointToMap(FineTerrainAnnotationDocument annotation, float modelX, float modelZ)
    {
        if (_document is null)
        {
            return PointF.Empty;
        ...
        FineTerrainWorldScale scale = annotation.WorldScale;
        double metersX = (modelX - scale.ModelCenter.X) * scale.XMetersPerModelUnit;
        double metersZ = (modelZ - scale.ModelCenter.Z) * scale.ZMetersPerModelUnit;
        float mapX = (float)(_document.Width * (0.5 + metersX / Math.Max(1e-6, _document.FieldLengthM)));
        float mapY = (float)(_document.Height * (0.5 + metersZ / Math.Max(1e-6, _document.FieldWidthM)));
        return new PointF(
            Math.Clamp(mapX, 0f, Math.Max(0f, _document.Width)),
            Math.Clamp(mapY, 0f, Math.Max(0f, _document.Height)));
    }
    ...
    private readonly record struct FacilitySelectionGroupResult(
        int CompositeCount,
        int ComponentCount,
