...
internal sealed class TerrainFacetRuntime
{
    public TerrainFacetRuntime(
        ...
        float collisionExpandM,
        float collisionHeightOffsetM,
        ...
        IReadOnlyList<float> heightsM)
    {
        ...
        CollisionExpandM = Math.Max(0f, collisionExpandM);
        CollisionHeightOffsetM = collisionHeightOffsetM;
    ...
    public float CollisionExpandM { get; }

    public float CollisionHeightOffsetM { get; }
...
internal sealed class RuntimeGridData
{
    public RuntimeGridData(
        ...
        TerrainCacheCollisionSurface? collisionSurface = null)
    {
    ...
    public TerrainCacheCollisionSurface? CollisionSurface { get; }
    ...
    public bool TrySampleFacetSurface(double worldX, double worldY, out float heightM, out Vector3 normal, out TerrainFacetRuntime? facet)
        => TrySampleFacetSurfaceInternal(worldX, worldY, collisionGeometry: false, out heightM, out normal, out facet);

    public bool TrySampleFacetCollisionSurface(double worldX, double worldY, out float heightM, out Vector3 normal, out TerrainFacetRuntime? facet)
        => TrySampleFacetSurfaceInternal(worldX, worldY, collisionGeometry: true, out heightM, out normal, out facet);

    private bool TrySampleFacetSurfaceInternal(
        double worldX,
        double worldY,
        bool collisionGeometry,
        out float heightM,
        out Vector3 normal,
        out TerrainFacetRuntime? facet)
    {
        facet = null;
        heightM = 0f;
        ...
        {
            if (collisionGeometry && !candidate.CollisionEnabled)
            {
                continue;
            }

            float padding = collisionGeometry ? candidate.CollisionExpandM : 0f;
            if (point.X < candidate.MinX || point.X > candidate.MaxX || point.Y < candidate.MinY || point.Y > candidate.MaxY)
            {
                if (padding <= 1e-5f
                    || point.X < candidate.MinX - padding
                    || point.X > candidate.MaxX + padding
                    || point.Y < candidate.MinY - padding
                    || point.Y > candidate.MaxY + padding)
                {
                    continue;
                }
            }

            if (!TrySampleFacetHeight(candidate, point, collisionGeometry, out float candidateHeight, out Vector3 candidateNormal))
            {
                continue;
            }
    ...
    public float SampleHeightWithFacets(double worldX, double worldY)
    {
        if (CollisionSurface is not null
            && CollisionSurface.TrySampleHeight(worldX, worldY, out TerrainSurfaceSample surfaceSample))
        {
            return surfaceSample.HeightM <= 0.02f ? 0f : surfaceSample.HeightM;
        }
    ...
    public float SampleCollisionHeightWithFacets(double worldX, double worldY)
    {
        if (CollisionSurface is not null
            && CollisionSurface.TrySampleHeight(worldX, worldY, out TerrainSurfaceSample surfaceSample))
        {
        ...
        float baseHeight = SampleSmoothedHeight(worldX, worldY);
        return TrySampleFacetCollisionSurface(worldX, worldY, out float facetHeight, out _, out _)
            ? Math.Max(baseHeight, facetHeight)
            : baseHeight;
    }
    ...
    public bool TrySampleCollisionSurface(double worldX, double worldY, out TerrainSurfaceSample sample, bool allowNeighborExpansion = true)
    {
        sample = default;
        return CollisionSurface is not null
            && CollisionSurface.TrySampleHeight(worldX, worldY, out sample, allowNeighborExpansion ? 1 : 0);
    }

    public bool TrySampleCollisionSurfaceBand(
        double worldX,
        double worldY,
        double minHeightM,
        double maxHeightM,
    ...
    public bool IsMovementBlockedAt(double worldX, double worldY, double referenceHeightM, double allowedRiseM)
    {
        if (CollisionSurface is not null)
        {
            return CollisionSurface.IsMovementBlocked(worldX, worldY, referenceHeightM, allowedRiseM);
        }

        int cellX = Math.Clamp((int)Math.Floor(worldX / Math.Max(1e-6f, CellWidthWorld)), 0, WidthCells - 1);
        int cellY = Math.Clamp((int)Math.Floor(worldY / Math.Max(1e-6f, CellHeightWorld)), 0, HeightCells - 1);
        return MovementBlockMap[IndexOf(cellX, cellY)];
    }

    public bool HasCollisionSurfaceWallContact(double worldX, double worldY, double minHeightM, double maxHeightM, int maxCellRadius = 0)
    {
        return CollisionSurface is not null
        ...
        out TerrainSurfaceRayHit hit)
    {
        hit = default;
        return CollisionSurface is not null
            && CollisionSurface.TryRaycast(startM, endM, metersPerWorldUnit, projectileRadiusM, out hit);
    }

    public int CollectCollisionDebugTriangles(
        double centerWorldX,
        double centerWorldY,
        double radiusWorld,
    ...
    {
        destination.Clear();
        return CollisionSurface is null
            ? 0
            : CollisionSurface.CollectDebugTriangles(centerWorldX, centerWorldY, radiusWorld, destination, maxTriangles);
    }
    ...
    private static bool TrySampleFacetHeight(
        TerrainFacetRuntime facet,
        Vector2 point,
        bool collisionGeometry,
        out float heightM,
        out Vector3 normal)
    {
        ...
        IReadOnlyList<Vector2> polygon = collisionGeometry
            ? BuildCollisionPolygon(facet)
            : facet.PointsWorld;
        float heightOffset = collisionGeometry ? facet.CollisionHeightOffsetM : 0f;
        Vector2 anchor = polygon[0];
        float anchorHeight = facet.HeightsM[0];
        for (int index = 1; index < polygon.Count - 1; index++)
        {
            Vector2 b = polygon[index];
            Vector2 c = polygon[index + 1];
            ...
            heightM = anchorHeight * barycentric.X + hb * barycentric.Y + hc * barycentric.Z + heightOffset;
            Vector3 p0 = new(anchor.X, anchorHeight + heightOffset, anchor.Y);
            Vector3 p1 = new(b.X, hb + heightOffset, b.Y);
            Vector3 p2 = new(c.X, hc + heightOffset, c.Y);
    ...
    private static IReadOnlyList<Vector2> BuildCollisionPolygon(TerrainFacetRuntime facet)
    {
        if (facet.CollisionExpandM <= 1e-5f || facet.PointsWorld.Count == 0)
        {
            return facet.PointsWorld;
        }
        ...
        Vector2[] expanded = new Vector2[facet.PointsWorld.Count];
        for (int index = 0; index < facet.PointsWorld.Count; index++)
        {
            Vector2 point = facet.PointsWorld[index];
            Vector2 direction = point - new Vector2(centerX, centerY);
            ...
            expanded[index] = point + direction / length * facet.CollisionExpandM;
        }
