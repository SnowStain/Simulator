...
internal sealed class Simulator3dOptions
{
    ...
    public string? PreviewRoleKey { get; init; }

    public string? PreviewSubtypeKey { get; init; }
    ...
    public static Simulator3dOptions Parse(string[] args)
    {
        ...
        string? previewRoleKey = null;
        string? previewSubtypeKey = null;
        ...
        for (int index = 0; index < args.Length; index++)
        {
            ...
            if ((current.Equals("--preview-role", StringComparison.OrdinalIgnoreCase)
                    || current.Equals("--appearance-role", StringComparison.OrdinalIgnoreCase))
                && index + 1 < args.Length)
            {
                previewRoleKey = NormalizePreviewRoleKey(args[++index]);
                continue;
            }

            if ((current.Equals("--preview-subtype", StringComparison.OrdinalIgnoreCase)
                    || current.Equals("--appearance-subtype", StringComparison.OrdinalIgnoreCase))
                && index + 1 < args.Length)
            {
                previewSubtypeKey = NormalizePreviewSubtypeKey(args[++index]);
                continue;
            }
        }
        ...
        return new Simulator3dOptions
        {
            ...
            PreviewRoleKey = previewRoleKey,
            PreviewSubtypeKey = previewSubtypeKey,
        };
    }
    ...
    {
        string mode = (selected ?? string.Empty).Trim().ToLowerInvariant();
        if (mode is "duel" or "1v1" or "duel_1v1" or "duel-1v1" or "vs1")
        {
            return "duel_1v1";
        }

        if (mode is "unit_test" or "unit-test" or "unittest" or "test_arena" or "test-arena")
        {
            return "unit_test";
        }
        ...
        if (mode is "map_component_test" or "map-component-test" or "componenttest" or "maptest")
        {
            return "map_component_test";
        }
    ...
    public static string? NormalizeEditorName(string? raw)
    {
        ...
        return value switch
        {
            ...
            "map_component_test" or "map-component-test" or "component_test" or "component-test" => "map_component_test",
    ...
    public static string? NormalizePreviewRoleKey(string? raw)
    {
        string value = (raw ?? string.Empty).Trim().ToLowerInvariant();
        if (string.IsNullOrWhiteSpace(value))
        {
    ...
    public static string? NormalizePreviewSubtypeKey(string? raw)
    {
