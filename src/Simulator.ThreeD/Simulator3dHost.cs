...
internal sealed class Simulator3dHost
{
    ...
    public void Step(PlayerControlState? playerControlState = null)
    {
        ...
        segmentStartTicks = System.Diagnostics.Stopwatch.GetTimestamp();
        _terrainMotionService.Step(World, _runtimeGrid, DeltaTimeSec, _aiEnabled);
    ...
    {
        SimulationWorldState world = _bootstrapService.BuildInitialWorld(_config, _rules, MapPreset);
        ApplyConfiguredRoleProfilesToWorld(world, resetHealth: true, _rules, CaptureLobbyWorldBuildSnapshot());
    ...
    {
        MapPresetDefinition mapPreset = _mapPresetService.LoadPreset(_layout, snapshot.ActiveMapPreset);
    ...
    private bool ShouldDeferInitialLobbyBootstrap(Simulator3dOptions options)
    {
        ...
        if (IsMapComponentTestMode || IsSingleUnitTestMode)
        {
            return false;
        }
