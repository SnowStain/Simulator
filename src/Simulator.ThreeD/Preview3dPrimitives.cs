...
internal static class Preview3dPrimitives
{
        ...
        float scale)
    {
        ...
        foreach (Preview3dFace face in faces)
        {
            ...
            Vector3 normal = Vector3.Normalize(Vector3.Cross(rotated[1] - rotated[0], rotated[2] - rotated[0]));
            float lighting = Math.Clamp(0.32f + Vector3.Dot(normal, light) * 0.55f + MathF.Abs(normal.Y) * 0.18f, 0.18f, 1.0f);
