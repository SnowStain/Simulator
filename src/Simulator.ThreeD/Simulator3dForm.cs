...
internal sealed partial class Simulator3dForm : Form
{
        ...
        bool externallyDrivenCompatibilityMode)
    {
        ...
        {
            QueueLobbyWorldRebuild("异步加载地图与世界中");
        }
    ...
    private void RenderFrameToGraphics(Graphics graphics, bool allowGpuScene)
    {
        ...
        if (_appState != SimulatorAppState.InMatch && _lobbyWorldRebuildTask is not null)
        {
    ...
    private void DrawLobbyWorldLoadingOverlay(Graphics graphics)
    {
        ...
        string mapLabel = FormatMapPresetLabel(_host.ActiveMapPreset);
        graphics.DrawString($"Map: {mapLabel}", _tinyHudFont, textBrush, new RectangleF(panel.X + 24, panel.Y + 124, panel.Width - 48, 22), centered);
        graphics.DrawString("UI is responsive. World bootstrap continues in background.", _tinyHudFont, mutedBrush, new RectangleF(panel.X + 24, panel.Y + 146, panel.Width - 48, 22), centered);
    }
    ...
    private void DrawMainMenu(Graphics graphics)
    {
        ...
        Rectangle panel = new((ClientSize.Width - panelWidth) / 2, 170, panelWidth, 348);
        DrawPanel(graphics, panel);

        graphics.DrawString("地图预设", _menuSubtitleFont, Brushes.Gainsboro, panel.X + 32, panel.Y + 30);
        graphics.DrawString("可用地图", _tinyHudFont, Brushes.Gainsboro, panel.X + 32, panel.Y + 70);
        DrawMainMenuMapChoices(graphics, panel.X + 32, panel.Y + 92, panel.Width - 64, panel.Bottom - 142);

        RectangleF helpRect = new(panel.X + 32, panel.Bottom - 142, panel.Width - 64, 42);
        using (var helpBrush = new SolidBrush(Color.FromArgb(214, 222, 230)))
        {
            graphics.DrawString(
                $"当前选择：{_host.ActiveMapPreset}。启动时会自动扫描 maps/ 与 map_presets/ 目录。",
                _smallHudFont,
                helpBrush,
                helpRect);
        }
        ...
        Rectangle componentTest = new(panel.X + 408, panel.Bottom - 120, 176, 34);
        Rectangle openLobby = new(panel.X + 32, panel.Bottom - 62, panel.Width - 64, 42);
        DrawButton(graphics, terrainEditor, "地图编辑器", "menu_open_terrain_editor", false, Color.FromArgb(80, 136, 198));
        DrawButton(graphics, appearanceEditor, "单位编辑器", "menu_open_appearance_editor", false, Color.FromArgb(108, 120, 204));
        DrawButton(graphics, componentTest, "地图组件测试", "menu_open_map_component_test", false, Color.FromArgb(92, 156, 118));
        DrawButton(graphics, openLobby, "进入大厅", "menu_open_lobby", true, Color.FromArgb(52, 132, 226));
    }
    ...
    private void DrawLobby(Graphics graphics)
    {
        ...
        graphics.DrawString($"地图  {_host.ActiveMapPreset}", _menuSubtitleFont, metaBrush, panel.X + 30, panel.Y + 24);
        graphics.DrawString($"阵营  {_host.SelectedTeam.ToUpperInvariant()}  （按 T 切换）", _tinyHudFont, metaBrush, panel.X + 30, panel.Y + 52);
        ...
        DrawLobbyOptionRow(
            ...
            new[]
            {
                ("On", "lobby_ai:on", _host.AiEnabled),
                ("Off", "lobby_ai:off", !_host.AiEnabled),
            });
        nextConfigY += 42;
        ...
        DrawButton(
            graphics,
            start,
            _lobbyWorldRebuildTask is null ? "Start Match" : "Updating...",
            "lobby_start_match",
    ...
    private void DrawCrosshairStatusProgressRings(Graphics graphics, float centerX, float centerY)
    {
        ...
        else if (entity.HeatLockTimerSec > 1e-6 || string.Equals(entity.State, "heat_locked", StringComparison.OrdinalIgnoreCase))
        {
            ...
            totalSec = Math.Max(0.5, remainingSec);
            label = "\u70ed\u91cf\u8d85\u9650";
            color = Color.FromArgb(255, 88, 88);
        }
        else
        ...
        float progress = !entity.IsAlive
            ? (float)Math.Clamp(1.0 - remainingSec / Math.Max(0.1, totalSec), 0.0, 1.0)
            : (float)Math.Clamp(remainingSec / Math.Max(0.1, totalSec), 0.0, 1.0);
        RectangleF ring = new(centerX - 44f, centerY - 44f, 88f, 88f);
        using var backPen = new Pen(Color.FromArgb(120, 12, 14, 18), 5.0f);
    ...
    private void DrawCentralQuarterGauges(Graphics graphics)
    {
        ...
        using var hudShadowBrush = new SolidBrush(Color.FromArgb(112, 0, 0, 0));
        graphics.DrawString(projectileSpeedText, _tinyHudFont, hudShadowBrush, textX + 1f, textY + 1f);
        graphics.DrawString(projectileSpeedText, _tinyHudFont, hudTextBrush, textX, textY);
        graphics.DrawString(allowedShotsText, _tinyHudFont, hudShadowBrush, textX + 1f, textY + 17f);
    ...
    private void DrawF3DebugPoseOverlay(Graphics graphics)
    {
        ...
        lines.Add($"速度 {speedMps:0.00}m/s  vx:{displayVx:+0.00;-0.00;0.00} vy:{displayVy:+0.00;-0.00;0.00}");
...
#pragma warning disable CS0162
    ...
    private void DrawVisionPoseSolveOverlay(Graphics graphics)
    {
        ...
        string plateSummary = $"plate={shooter.AutoAimPlateId}";
        ...
        float y = panel.Y + 10;
        graphics.DrawString("F8 Vision Solve", _smallHudFont, titleBrush, panel.X + 12, y);
        y += 22f;
        graphics.DrawString($"id={target.Id} {plateSummary} type={targetTypeLabel}", _tinyHudFont, textBrush, panel.X + 12, y);
        y += 18f;
        graphics.DrawString($"source={sourceLabel}", _tinyHudFont, textBrush, panel.X + 12, y);
        y += 18f;
        graphics.DrawString($"range={rangeM:0.00}m yaw={observedYawDeg:0.0}deg pitch={observedPitchDeg:0.0}deg", _tinyHudFont, textBrush, panel.X + 12, y);
        y += 18f;
        graphics.DrawString(
            $"v=({shooter.AutoAimObservedVelocityXMps:0.00},{shooter.AutoAimObservedVelocityYMps:0.00},{shooter.AutoAimObservedVelocityZMps:0.00})m/s |v|={observedSpeedMps:0.00}m/s",
            _tinyHudFont,
            textBrush,
            panel.X + 12,
            y);
        y += 18f;
        graphics.DrawString(
            $"omega={shooter.AutoAimObservedAngularVelocityRadPerSec:0.000}rad/s lead={shooter.AutoAimLeadTimeSec:0.000}s eff={effectiveLeadTimeSec:0.000}s dist={shooter.AutoAimLeadDistanceM:0.00}m",
            _tinyHudFont,
            textBrush,
            panel.X + 12,
            y);
        y += 18f;
        graphics.DrawString(
            $"aim=({shooter.AutoAimAimPointX:0.00},{shooter.AutoAimAimPointY:0.00},{shooter.AutoAimAimPointHeightM:0.00}) corr=({yawCorrectionDeg:+0.00;-0.00;0.00},{pitchCorrectionDeg:+0.00;-0.00;0.00})deg",
            _tinyHudFont,
            textBrush,
            panel.X + 12,
            y);
        ...
        string compensationLine = compensation is null
            ? "comp=n/a"
            : $"comp={compensation.Value.Name} ts={compensation.Value.TranslationLeadScale:0.000} as={compensation.Value.AngularLeadScale:0.000} tb={compensation.Value.TimeBiasSec:+0.000;-0.000;0.000}s";
        graphics.DrawString(compensationLine, _tinyHudFont, textBrush, panel.X + 12, y);
        y += 18f;
        graphics.DrawString(
            $"solve=({shooter.AutoAimSmoothedYawDeg:0.00},{shooter.AutoAimSmoothedPitchDeg:0.00})deg acc={shooter.AutoAimAccuracy:P0} dist={shooter.AutoAimDistanceCoefficient:0.00} motion={shooter.AutoAimMotionCoefficient:0.00}",
            _tinyHudFont,
            textBrush,
            panel.X + 12,
            y);
        ...
        if (string.Equals(shooter.AutoAimTargetKind, "energy_disk", StringComparison.OrdinalIgnoreCase))
        {
            ...
            graphics.DrawString($"energy vision center + disks={diskCount} tracked={trackedIndex} {stateLine}", _tinyHudFont, debugBrush, panel.X + 12, panel.Bottom + 8);
        }
    }
    ...
    private static string ResolveVisionObservationSourceLabel(string? targetKind)
    {
        if (string.Equals(targetKind, "energy_disk", StringComparison.OrdinalIgnoreCase))
        {
            return "visual disk pose + inferred rotor motion";
        }

        if (string.Equals(targetKind, "outpost_armor", StringComparison.OrdinalIgnoreCase))
        {
            return "visual armor pose + outpost ring prior";
        }

        if (string.Equals(targetKind, "base_armor", StringComparison.OrdinalIgnoreCase))
        {
            return "visual armor pose + base geometry prior";
        }

        return "visual armor pose + chassis prior";
    }
    ...
    private void DrawPlayerStatusPanelV2(Graphics graphics)
    {
        ...
        graphics.DrawString(ResolveHudRobotTitle(entity), _smallHudFont, titleBrush, rightX, panel.Y + 10f);
    ...
    private void DrawRobotPortraitNeo(Graphics graphics, SimulationEntity entity, PointF center, float radius, Color teamColor)
    {
        ...
        RectangleF levelBadge = new(center.X - 16f, center.Y + radius - 20f, 32f, 32f);
        using var badge = new SolidBrush(Color.FromArgb(232, 22, 26, 33));
        using var badgePen = new Pen(Color.FromArgb(220, 245, 248, 252), 1.2f);
        using StringFormat centerFormat = new() { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center };
        graphics.FillEllipse(badge, levelBadge);
        graphics.DrawEllipse(badgePen, levelBadge);
        graphics.DrawString(ResolveHudRobotIndex(entity).ToString(), _hudMidFont, Brushes.WhiteSmoke, levelBadge, centerFormat);
    }
        ...
        try
        {
            ...
            float previewExtent = Math.Max(
                ...
                Math.Max(
                    ...
                    Math.Max(profile.BodyWidthM, profile.GimbalHeightM + profile.BodyClearanceM)));
            _cameraTargetM = new Vector3(0f, Math.Max(0.22f, profile.BodyClearanceM + profile.BodyHeightM * 0.55f), 0f);
            float distance = Math.Clamp(previewExtent * 2.2f, 1.35f, 4.2f);
            _cameraPositionM = _cameraTargetM + new Vector3(distance * 0.72f, distance * 0.46f, distance * 0.92f);
            _viewMatrix = Matrix4x4.CreateLookAt(_cameraPositionM, _cameraTargetM, Vector3.UnitY);
            float aspect = Math.Max(0.6f, viewport.Width / (float)Math.Max(1, viewport.Height));
            _projectionMatrix = Matrix4x4.CreatePerspectiveFieldOfView(0.78f, aspect, 0.02f, 40f);
    ...
    private string ResolveHudRobotTitle(SimulationEntity entity)
        => $"{ResolveHudRobotIndex(entity)}号 {ResolveRoleLabel(entity)}";
    ...
    private void DrawHeroDeploymentFeedOverlay(Graphics graphics)
    {
        ...
        string sideDetail = exiting
            ? "Auto lob fire is active. Hold Z to exit deployment."
            : "Deployment forces lob auto-aim and auto fire. Manual Q switching still works outside deployment.";
        DrawRightSideHudCard(
            graphics,
            sideBox,
    ...
    private void DrawDeploymentPrompt(Graphics graphics)
    {
        ...
        string deployDetail = entity.HeroDeploymentRequested
            ? "Hold position in the zone until the 2s deployment readout completes."
            : "Hold Z for 2s to enter deployment. Deployment adds auto fire; Q can switch normal/lob any time.";
        DrawRightSideHudCard(
            graphics,
            rightBox,
...
#pragma warning restore CS0162
    ...
    private void LogMatchStartupState(string message)
    {
        try
        {
            ...
            Directory.CreateDirectory(logDirectory);
            File.AppendAllText(
                Path.Combine(logDirectory, "match_startup.log"),
                $"{DateTime.Now:HH:mm:ss.fff} phase={_matchStartupPhase} paused={_paused} {message}{Environment.NewLine}");
        }
        catch
        {
        }
    ...
    private void UpdateCameraMatrices()
    {
        ...
        if (_firstPersonView && selected is not null)
        {
            ...
            if (selected.AutoAimLocked
                && selected.AutoAimAimPointHeightM > 1e-6
                && !selected.AutoAimGuidanceOnly
                && !IsHeroLobModeActive(selected))
            {
                Vector3 lockedAimPoint = ToScenePoint(selected.AutoAimAimPointX, selected.AutoAimAimPointY, (float)selected.AutoAimAimPointHeightM);
                Vector3 aimDirection = lockedAimPoint - eye;
                if (aimDirection.LengthSquared() > 1e-6f)
                {
                    sightConvergence = lockedAimPoint;
                }
            }
    ...
    private void DrawTeamTopNeonLights(Graphics graphics)
    {
        ...
        graphics.SmoothingMode = SmoothingMode.AntiAlias;
        DrawTeamTopNeonBand(graphics, "red", farSide: false);
        DrawTeamTopNeonBand(graphics, "blue", farSide: true);
    ...
    private bool HasFineTerrainAnnotationMap()
        => !_previewOnly
            && !string.IsNullOrWhiteSpace(_host.MapPreset.AnnotationPath)
            && File.Exists(_host.MapPreset.AnnotationPath);
    ...
    private bool IsHeroLobReticleAlignedForAutoFire(SimulationEntity entity)
    {
        ...
        if (TryGetHeroLobCalibrationPreviewCached(entity, out HeroLobCalibrationPreview preview, includeFireWindowSuggestion: false))
        {
            ...
            float centerHorizontalToleranceM = Math.Max(0.025f, preview.PlateWidthM * 0.16f);
            float centerVerticalToleranceM = Math.Max(0.025f, preview.PlateHeightM * 0.18f);
            float depthToleranceM = 0.08f + Math.Clamp((float)entity.AutoAimLeadDistanceM * 0.012f, 0f, 0.08f);
            ...
            if (TryGetProjectedHeroLobPlatePolygon(entity, out PointF[] projectedPlateForWindow))
            {
                float centerMarginPx = Math.Clamp(
                    14f + (float)entity.AutoAimLeadDistanceM * 1.2f,
                    14f,
                    _firstPersonView ? 32f : 40f);
            ...
            if (TryGetAutoAimProjectedPoint(entity, out PointF projectedAimForWindow))
            {
                ...
                float previewFallbackThresholdPx = _firstPersonView ? 26f : 34f;
        ...
        if (TryGetProjectedHeroLobPlatePolygon(entity, out PointF[] projectedPlate))
        {
            ...
            float overlapMarginPx = Math.Clamp(
                longestEdge * 0.16f
                + (_firstPersonView ? 14f : 18f)
                + Math.Clamp((float)entity.AutoAimLeadDistanceM * 0.9f, 0f, 10f),
                _firstPersonView ? 16f : 22f,
                _firstPersonView ? 34f : 46f);
        ...
        float fallbackThresholdPx = (_firstPersonView ? 24f : 32f)
            + (entity.HeroDeploymentActive ? 6f : 0f)
            + Math.Clamp((float)entity.AutoAimLeadDistanceM * 0.8f, 0f, 8f);
    ...
    private PlayerControlState BuildPlayerControlState(bool forceEnable = false)
    {
        ...
        bool manualFirePressed = energyAutoAimSingleShot ? _pendingSingleFireRequest : _firePressed;
    ...
    private void CaptureExperienceGainNotifications()
    {
        foreach (SimulationEntity entity in _host.World.Entities)
        {
            ...
            if (gainedExperience > 1e-3)
            {
                ...
                {
                    AppendMatchEvent($"{FormatEventEntityName(entity)} +{gainedExperience:0} XP", teamColor, 3.5f);
                }
            }
            ...
            if (levelUps > 0)
            {
                _centerBuffToasts.Add(new CenterBuffToast(
                    "等级提升",
                    $"{FormatEventEntityName(entity)}  Lv{Math.Max(1, entity.Level)}",
                    teamColor,
                    2.8f));
            }
