...
internal sealed partial class Simulator3dForm
{
        ...
        out HeroLobCalibrationPreview preview)
    {
        ...
        if (Math.Abs(_host.World.GameTimeSec - _heroLobCalibrationPreviewCacheWorldTimeSec) > 0.075
            ...
            || Math.Abs(shooter.TurretYawDeg - _heroLobCalibrationPreviewCacheTurretYawDeg) > 0.35
            || Math.Abs(shooter.GimbalPitchDeg - _heroLobCalibrationPreviewCacheGimbalPitchDeg) > 0.28)
        {
            return false;
        }
...
#pragma warning disable CS0162
    ...
    {
        {
            SimulationEntity? localizedShooter = _host.SelectedEntity;
            if (localizedShooter is null
                || !string.Equals(localizedShooter.RoleKey, "hero", StringComparison.OrdinalIgnoreCase)
                || !SimulationCombatMath.IsHeroLobAutoAimMode(localizedShooter)
                || !ShouldShowHeroLobSubview(localizedShooter))
            {
                return;
            }

            Rectangle localizedPanel = new(20, ClientSize.Height - 344, 592, 178);
            Rectangle localizedPreviewRect = GetHeroLobSubviewRect();
            if (localizedShooter.HeroDeploymentActive)
            {
                return;
            }
            using GraphicsPath localizedPath = CreateRoundedRectangle(localizedPanel, 6);
            using var localizedFill = new SolidBrush(Color.FromArgb(220, 10, 16, 24));
            using var localizedBorder = new Pen(Color.FromArgb(160, 112, 134, 156), 1f);
            using (var localizedFillRegion = new Region(localizedPath))
            {
                localizedFillRegion.Exclude(localizedPreviewRect);
                graphics.FillRegion(localizedFill, localizedFillRegion);
            }

            graphics.DrawPath(localizedBorder, localizedPath);

            using var localizedTitleBrush = new SolidBrush(Color.FromArgb(238, 244, 248));
            using var localizedTextBrush = new SolidBrush(Color.FromArgb(206, 216, 226));
            graphics.DrawString("吊射弹道校准", _smallHudFont, localizedTitleBrush, localizedPanel.X + 14, localizedPanel.Y + 10);
            string localizedModeLine = _autoAimAssistMode == AutoAimAssistMode.GuidanceOnly
                ? "引导模式：只显示预测命中点和发射窗口，不接管云台或火控。"
                : "硬锁/部署：只有命中窗口、中心误差与法线夹角同时满足时才自动开火。";

            bool localizedTrackingSubviewTarget = IsHeroLobSubviewTrackingTarget(localizedShooter);
            if (!TryGetHeroLobCalibrationPreviewCached(localizedShooter, out HeroLobCalibrationPreview localizedPreview, includeFireWindowSuggestion: true))
            {
                DrawHeroLobSubviewFrame(graphics, localizedPreviewRect, hasPreview: false, locked: localizedTrackingSubviewTarget);
                graphics.DrawString("右键锁定前哨站或基地装甲板后，这里会显示当前弹道的命中点或偏移量。", _tinyHudFont, localizedTextBrush, localizedPanel.X + 14, localizedPanel.Y + 38);
                graphics.DrawString("普通模式仅处理 8m 内车体装甲板；吊射模式全距离处理前哨站/基地装甲板。", _tinyHudFont, localizedTextBrush, localizedPanel.X + 14, localizedPanel.Y + 58);
                graphics.DrawString(localizedModeLine, _tinyHudFont, localizedTextBrush, localizedPanel.X + 14, localizedPanel.Y + 78);
                return;
            }

            DrawHeroLobSubviewFrame(graphics, localizedPreviewRect, hasPreview: true, locked: localizedTrackingSubviewTarget);

            Color localizedAccentColor = localizedPreview.HitsPlate
                ? Color.FromArgb(92, 224, 144)
                : Color.FromArgb(255, 198, 96);
            using var localizedAccentBrush = new SolidBrush(localizedAccentColor);
            string localizedSummary = localizedPreview.HitsPlate
                ? $"命中 {localizedPreview.TargetId}/{localizedPreview.PlateId}"
                : $"未命中 {localizedPreview.TargetId}/{localizedPreview.PlateId}";
            graphics.DrawString(localizedSummary, _tinyHudFont, localizedAccentBrush, localizedPanel.X + 204, localizedPanel.Y + 40);
            string localizedFireWindowText = localizedPreview.FireWindowReady
                ? "窗口：允许发射"
                : "窗口：等待装甲板正面/中心窗口";
            graphics.DrawString($"{localizedFireWindowText}   法线夹角 {localizedPreview.NormalAngleDeg:0.0}° / 45°", _tinyHudFont, localizedPreview.FireWindowReady ? localizedAccentBrush : localizedTextBrush, localizedPanel.X + 204, localizedPanel.Y + 118);
            string localizedFireAdvice = localizedPreview.FireWindowReady
                ? "建议：现在发射"
                : localizedPreview.HasSuggestedFireWindow
                    ? $"建议：等待 {localizedPreview.SecondsToFireWindow:0.00}s 后再发射"
                    : "建议：当前姿态 2.4s 内无合适窗口";
            graphics.DrawString(localizedFireAdvice, _tinyHudFont, localizedPreview.FireWindowReady ? localizedAccentBrush : localizedTextBrush, localizedPanel.X + 204, localizedPanel.Y + 138);
            graphics.DrawString(localizedModeLine, _tinyHudFont, localizedTextBrush, localizedPanel.X + 204, localizedPanel.Y + 158);
            if (localizedPreview.HitsPlate)
            {
                graphics.DrawString($"命中点：右偏 {localizedPreview.HorizontalOffsetM:+0.000;-0.000;0.000}m   高偏 {localizedPreview.VerticalOffsetM:+0.000;-0.000;0.000}m", _tinyHudFont, localizedTextBrush, localizedPanel.X + 204, localizedPanel.Y + 62);
                graphics.DrawString($"装甲板 {localizedPreview.PlateWidthM:0.000}m x {localizedPreview.PlateHeightM:0.000}m", _tinyHudFont, localizedTextBrush, localizedPanel.X + 204, localizedPanel.Y + 82);
            }
            else
            {
                string localizedMissMode = localizedPreview.CrossedPlatePlane ? "穿过板平面但未落在板面内" : "未穿过板平面";
                graphics.DrawString($"{localizedMissMode}   横偏 {localizedPreview.HorizontalOffsetM:+0.000;-0.000;0.000}m   高偏 {localizedPreview.VerticalOffsetM:+0.000;-0.000;0.000}m   深度 {localizedPreview.DepthOffsetM:+0.000;-0.000;0.000}m   总偏差 {localizedPreview.TotalOffsetM:0.000}m", _tinyHudFont, localizedTextBrush, new RectangleF(localizedPanel.X + 204, localizedPanel.Y + 58, localizedPanel.Width - 218, 42));
            }

            DrawHeroLobCalibrationWorldMarker(graphics, localizedPreview);
            return;
        }
...
#pragma warning restore CS0162
    ...
    private bool ShouldShowHeroLobSubview(SimulationEntity shooter)
    {
        return IsFirstPersonHudVisible()
            && string.Equals(shooter.RoleKey, "hero", StringComparison.OrdinalIgnoreCase)
            && SimulationCombatMath.IsHeroLobAutoAimMode(shooter)
            && (shooter.HeroDeploymentActive || _autoAimPressed);
    }

    private bool IsHeroDeploymentSubviewOnlyMode()
    {
        SimulationEntity? shooter = _host.SelectedEntity;
        return IsFirstPersonHudVisible()
            && shooter is not null
    ...
    private bool ShouldBlackoutHeroDeploymentViewport(SimulationEntity shooter)
    {
        return IsHeroDeploymentSubviewOnlyMode();
    }

    private void DrawHeroDeploymentViewportMask(Graphics graphics)
    {
        SimulationEntity? shooter = _host.SelectedEntity;
        if (shooter is null || !ShouldBlackoutHeroDeploymentViewport(shooter))
        {
            return;
        }

        using var shade = new SolidBrush(Color.FromArgb(244, 0, 0, 0));
...
#pragma warning disable CS0162
    ...
    {
        {
            ...
            Rectangle previewRect = GetHeroLobSubviewPresentationRect();
            bool trackingSubviewTarget = IsHeroLobSubviewTrackingTarget(shooter);

            DrawHeroLobSubviewFrame(graphics, previewRect, hasPreview: true, locked: trackingSubviewTarget);
            if (shooter.HeroDeploymentActive)
            {
                using var titleBrush = new SolidBrush(Color.FromArgb(246, 248, 248, 248));
                ...
                RectangleF labelRect = new(previewRect.X, previewRect.Y + 14f, previewRect.Width, 22f);
                graphics.DrawString("部署模式断电", _smallHudFont, shadowBrush, new RectangleF(labelRect.X + 1f, labelRect.Y + 1f, labelRect.Width, labelRect.Height), center);
                graphics.DrawString("部署模式断电", _smallHudFont, titleBrush, labelRect, center);
                DrawHeroLobDeploymentTrajectoryOverlay(graphics, shooter, previewRect);
            }

            if (!shooter.HeroDeploymentActive
                && TryGetHeroLobCalibrationPreviewCached(shooter, out HeroLobCalibrationPreview preview, includeFireWindowSuggestion: true))
            {
                DrawHeroLobCalibrationWorldMarker(graphics, preview);
            }
        ...
        SimulationEntity? activeShooter = _host.SelectedEntity;
        if (activeShooter is null
            || !string.Equals(activeShooter.RoleKey, "hero", StringComparison.OrdinalIgnoreCase)
            || !SimulationCombatMath.IsHeroLobAutoAimMode(activeShooter)
            || !ShouldShowHeroLobSubview(activeShooter))
        {
            return;
        }

        Rectangle activePreviewRect = GetHeroLobSubviewRect();
        bool activeTrackingSubviewTarget = IsHeroLobSubviewTrackingTarget(activeShooter);
        DrawHeroLobSubviewFrame(graphics, activePreviewRect, hasPreview: true, locked: activeTrackingSubviewTarget);

        if (activeShooter.HeroDeploymentActive)
        {
            using var deploymentFill = new LinearGradientBrush(
                activePreviewRect,
                Color.FromArgb(172, 224, 224, 224),
                Color.FromArgb(118, 28, 28, 28),
                LinearGradientMode.Vertical);
            using var deploymentShade = new SolidBrush(Color.FromArgb(78, 14, 14, 14));
            using var titleBrush = new SolidBrush(Color.FromArgb(246, 248, 248, 248));
            using var detailBrush = new SolidBrush(Color.FromArgb(224, 236, 236, 236));
            using var center = new StringFormat { Alignment = StringAlignment.Center };
            graphics.FillRectangle(deploymentFill, activePreviewRect);
            graphics.FillRectangle(deploymentShade, activePreviewRect);
            graphics.DrawString("部署模式断电", _smallHudFont, titleBrush, new RectangleF(activePreviewRect.X, activePreviewRect.Y + 14f, activePreviewRect.Width, 22f), center);
            graphics.DrawString("全屏副视角已接管显示，主视角停用。", _tinyHudFont, detailBrush, new RectangleF(activePreviewRect.X, activePreviewRect.Bottom - 28f, activePreviewRect.Width, 16f), center);
            DrawHeroLobDeploymentTrajectoryOverlay(graphics, activeShooter, activePreviewRect);
        }

        if (TryGetHeroLobCalibrationPreviewCached(activeShooter, out HeroLobCalibrationPreview activePreview, includeFireWindowSuggestion: true))
        {
            DrawHeroLobCalibrationWorldMarker(graphics, activePreview);
        }
...
#if false
        {
            ...
            if (shooter.HeroDeploymentActive)
            {
                ...
                using var detailBrush = new SolidBrush(Color.FromArgb(224, 236, 236, 236));
                using var center = new StringFormat { Alignment = StringAlignment.Center };
                graphics.FillRectangle(deploymentFill, previewRect);
                graphics.FillRectangle(deploymentShade, previewRect);
                graphics.DrawString("部署模式断电", _smallHudFont, titleBrush, new RectangleF(previewRect.X, previewRect.Y + 10f, previewRect.Width, 22f), center);
                graphics.DrawString("副视角保持追踪，主视角断开。", _tinyHudFont, detailBrush, new RectangleF(previewRect.X, previewRect.Bottom - 26f, previewRect.Width, 16f), center);
            }
    ...
    {
        int width = Math.Max(360, Math.Min(460, ClientSize.Width / 4));
        int height = Math.Max(216, (int)Math.Round(width * 0.6));
        int margin = 20;
        Rectangle hudPanel = GetPlayerStatusPanelRect();
        int x = hudPanel.X;
        int y = Math.Max(margin, hudPanel.Y - height - 14);
        if (x + width > ClientSize.Width - margin)
        {
            x = Math.Max(margin, ClientSize.Width - width - margin);
        }
...
#pragma warning restore CS0162
    ...
    {
        return GetHeroLobOverlayPanelRect();
    }

    private Rectangle GetHeroLobSubviewPresentationRect()
    {
        return IsHeroDeploymentSubviewOnlyMode()
            ? ClientRectangle
            : GetHeroLobSubviewRect();
    }
    ...
    private void DrawHeroLobSubviewFrame(Graphics graphics, Rectangle rect, bool hasPreview, bool locked)
    {
        ...
        {
            using var fullscreenBorder = new Pen(Color.FromArgb(hasPreview ? 122 : 88, locked ? 232 : 180, locked ? 236 : 196, locked ? 242 : 212), 1.0f);
            graphics.DrawRectangle(fullscreenBorder, rect.X, rect.Y, Math.Max(1, rect.Width - 1), Math.Max(1, rect.Height - 1));
            return;
        }
    ...
    private void DrawHeroLobSecondaryViewport(Graphics graphics)
    {
        ...
        SimulationEntity? shooter = _host.SelectedEntity;
        if (shooter is null || !ShouldShowHeroLobSubview(shooter))
        {
            return;
        }
        ...
        out float verticalFovRad)
    {
        ...
        Size logicalViewportSize = viewport.Size;
    ...
    private void DrawHeroLobDeploymentTrajectoryOverlay(Graphics graphics, SimulationEntity shooter, Rectangle previewRect)
    {
        if (!shooter.HeroDeploymentActive)
        {
            return;
        ...
        Span<Vector3> scenePoints = stackalloc Vector3[96];
        if (!TryBuildHeroLobTrajectoryScenePoints(shooter, scenePoints, out int pointCount))
        {
            return;
        }

        using var shadowPen = new Pen(Color.FromArgb(144, 0, 0, 0), 3.2f)
        {
            StartCap = LineCap.Round,
            EndCap = LineCap.Round,
            LineJoin = LineJoin.Round,
        };
        using var trajectoryPen = new Pen(Color.FromArgb(234, 248, 248, 248), 2.0f)
        {
            StartCap = LineCap.Round,
            EndCap = LineCap.Round,
            LineJoin = LineJoin.Round,
        ...
        if (UseGpuRenderer && !UseFastFlatRenderer)
        {
            ...
            if (!TryResolveHeroLobSubviewCrop(mainViewport, out Rectangle destinationRect, out Rectangle sourceRect))
            {
                return;
            }

            PointF[] mapped = new PointF[pointCount];
            int mappedCount = 0;
            for (int index = 0; index < pointCount; index++)
            {
                if (!TryProject(scenePoints[index], out PointF mainPoint, out _))
                {
                    continue;
                }

                if (!TryMapSubviewSourcePointToViewport(mainPoint, sourceRect, destinationRect, out PointF mappedPoint))
                {
                    continue;
                }

                mapped[mappedCount++] = mappedPoint;
            }

            if (mappedCount >= 2)
            {
                graphics.DrawLines(shadowPen, mapped[..mappedCount]);
                graphics.DrawLines(trajectoryPen, mapped[..mappedCount]);
            }
        ...
        try
        {
            ...
            PointF[] projected = new PointF[pointCount];
            int projectedCount = 0;
            for (int index = 0; index < pointCount; index++)
            {
                if (!TryProject(scenePoints[index], out PointF point, out _))
                {
                    continue;
                }

                projected[projectedCount++] = point;
            }

            if (projectedCount >= 2)
            {
                graphics.DrawLines(shadowPen, projected[..projectedCount]);
                graphics.DrawLines(trajectoryPen, projected[..projectedCount]);
            }
        }
        finally
        {
    ...
    private static bool TryMapSubviewSourcePointToViewport(PointF sourcePoint, Rectangle sourceRect, Rectangle destinationRect, out PointF mappedPoint)
    {
        mappedPoint = default;
        if (sourceRect.Width <= 1 || sourceRect.Height <= 1 || destinationRect.Width <= 1 || destinationRect.Height <= 1)
        {
            return false;
        }

        float u = (sourcePoint.X - sourceRect.Left) / Math.Max(1f, sourceRect.Width);
        float v = (sourcePoint.Y - sourceRect.Top) / Math.Max(1f, sourceRect.Height);
        if (!float.IsFinite(u) || !float.IsFinite(v))
        {
            return false;
        }

        mappedPoint = new PointF(
            destinationRect.Left + u * destinationRect.Width,
            destinationRect.Top + v * destinationRect.Height);
        return true;
    }

    private bool TryBuildHeroLobTrajectoryScenePoints(SimulationEntity shooter, Span<Vector3> points, out int count)
    {
        count = 0;
        double metersPerWorldUnit = Math.Max(_host.World.MetersPerWorldUnit, 1e-6);
        ...
        for (double t = 0.0; t <= maxLifeSec && count < points.Length; t += dt)
        {
            points[count++] = ToScenePoint(x, y, (float)heightM);
            ...
            if (runtimeGrid is not null
                && runtimeGrid.IsValid
                && t > 0.05
                && heightM <= runtimeGrid.SampleOcclusionHeight((float)x, (float)y) + 0.015)
            {
                if (count < points.Length)
                {
                    points[count++] = ToScenePoint(x, y, runtimeGrid.SampleOcclusionHeight((float)x, (float)y));
                }
    ...
    private List<BuffProgressEntry> CollectBuffProgressEntries(SimulationEntity entity)
    {
        ...
        AddTimedBuff(entries, "terrain_highland_def", "defense", "\u9ad8\u5730\u9632\u5fa1", "\u53d7\u5230\u4f24\u5bb3 x0.50", entity.TerrainHighlandDefenseTimerSec, 30.0, Color.FromArgb(104, 190, 255), 0.50);
    ...
    private float ResolveTerrainDetailRadiusM()
    {
        ...
        return Math.Clamp(_cameraDistanceM * 0.72f, 8.0f, 16.0f);
    }
