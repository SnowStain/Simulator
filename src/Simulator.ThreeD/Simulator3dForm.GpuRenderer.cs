...
internal sealed partial class Simulator3dForm
{
    ...
    private void DrawGpuMatch(Graphics graphics)
    {
        ...
        try
        {
            ...
            if (useSceneTexture && _glBindFramebuffer is not null)
            {
                ...
                {
                    PresentGpuTextureRegion(_gpuSceneColorTexture, sceneRenderSize, heroSubviewSourceRect, heroSubviewViewport, useNearestFilter: true);
                }
            }
            else
            {
        ...
        out int projectileVertices)
    {
        ...
        try
        {
            ...
            if (_host.SelectedEntity is SimulationEntity debugEntity)
            {
    ...
    private void DrawGpuHeroLobSecondaryViewport(Graphics graphics)
    {
        ...
        PresentGpuHeroLobSubviewTexture(viewport, sourceRect);
    }
        ...
        out Matrix4x4 projectionMatrix)
    {
        ...
        int extraBottom = Math.Max(0, desiredSourceRect.Bottom - mainViewport.Height);
        int maxUsefulExtra = Math.Max(viewport.Height * 2, mainViewport.Height / 3);
        int paddedExtra = Math.Min(extraBottom + 16, maxUsefulExtra);
    ...
    private bool TryBuildHeroLobSubviewSourceRequest(Rectangle mainViewport, out Rectangle viewport, out Rectangle desiredSourceRect)
    {
        ...
        if (TryResolveHeroLobSubviewCropCenterAndZoom(shooter, viewport.Size, mainViewport, out PointF resolvedCenter, out float resolvedZoomScale))
        {
            cropCenter = resolvedCenter;
            zoomScale = resolvedZoomScale;
        }

        int sourceWidth = Math.Max(32, (int)MathF.Round(viewport.Width / Math.Max(1.0f, zoomScale)));
        int sourceHeight = Math.Max(32, (int)MathF.Round(viewport.Height / Math.Max(1.0f, zoomScale)));
        ...
        out float zoomScale)
    {
            ...
            && TryGetProjectedHeroLobPlatePolygon(shooter, out PointF[] polygon))
        {
            ...
            if (bounds.Width > 2f && bounds.Height > 2f)
            {
                ...
                float targetArea = Math.Max(16f, bounds.Width * bounds.Height);
                ...
                zoomScale = Math.Clamp(MathF.Min(zoomX, zoomY), 3f, 14f);
                return true;
            }
        }
    ...
    private void PresentGpuHeroLobSubviewTexture(Rectangle destinationRect, Rectangle sourceRect)
    {
        ...
        int copyY = Math.Max(0, ClientSize.Height - sourceRect.Bottom);
        if (_gpuHeroLobSubviewTextureSize != sourceRect.Size)
        {
            glCopyTexImage2D(GlTexture2D, 0, GlRgba, sourceRect.X, copyY, sourceRect.Width, sourceRect.Height, 0);
    ...
    private void DrawGpuTeamTopNeonLights()
    {
        ...
        DrawGpuTeamTopNeonBand("red", farSide: false);
        DrawGpuTeamTopNeonBand("blue", farSide: true);
    }
