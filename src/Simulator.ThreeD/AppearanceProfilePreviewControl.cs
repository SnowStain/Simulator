...
internal sealed class AppearanceProfilePreviewControl : Control
{
    ...
    private void DrawThreeDView(Graphics graphics, Rectangle rect, RobotAppearanceProfileDefinition profile)
    {
        ...
        (Vector3 min, Vector3 max) = EditorPreviewGeometry.MeasureBounds(faces);
        ...
        float scale = Math.Min(viewport.Width / span, viewport.Height / Math.Max(0.4f, height + span * 0.18f)) * 0.76f;
        Vector3 center = new(
            (min.X + max.X) * 0.5f,
            min.Y + height * 0.38f,
            (min.Z + max.Z) * 0.5f);
        ...
        graphics.DrawString("3D preview now follows the runtime appearance profile geometry.", SystemFonts.DefaultFont, text, viewport.X + 8, viewport.Y + 8);
    }
    ...
    private static void BuildRobotPreview(ICollection<Preview3dFace> faces, RobotAppearanceProfileDefinition profile)
    {
        ...
        EditorPreviewGeometry.AppendAppearancePreview(faces, runtimeProfile);
    }
