...
internal sealed class TerrainMotionService
{
    ...
    private const double HeroLobStructureAutoAimReuseSec = 0.075;
        ...
        out AutoAimSolution solution)
    {
        ...
        if (!heroLobStructureCache && SimulationCombatMath.IsRotatingArmorPlate(target, plate))
        {
            target = null;
            plate = default;
            return false;
        ...
        out AutoAimSolution solution)
    {
        ...
        StoreAutoAimSolution(shooter, target, plate, plate, retainedObservationState, solution, world.GameTimeSec);
        return true;
    }
        ...
        double dt)
    {
        ...
        else if (energyDiskTarget && !entity.AutoAimGuidanceOnly)
        {
            entity.AutoAimSmoothedYawDeg = desiredYaw;
            entity.AutoAimSmoothedPitchDeg = desiredPitch;
        }
        else
        {
            double tau = energyDiskTarget
                ? 0.035
                : (lockChanged ? (sameTargetSwitch ? 0.070 : 0.155) : 0.105);
            double response = energyDiskTarget && !entity.AutoAimGuidanceOnly && !lockChanged
                ? 1.0
                : 1.0 - Math.Exp(-Math.Clamp(dt, 0.005, 0.08) / tau);
        ...
        double dt)
    {
        ...
        {
            bool ignoreWeakSlopeWallTrend =
                runtimeGrid.CollisionSurface is not null
                && string.Equals(trendContact.Reason, "collision_surface_wall_contact", StringComparison.OrdinalIgnoreCase)
                && trendContact.PenetrationWorld <= 0.016
                && supportHeight <= currentHeight + maxStep + jumpClearance + 0.08;
            if (ignoreWeakSlopeWallTrend)
            {
        ...
        double allowedRiseM)
    {
        ...
        try
        {
            string logDirectory = Path.Combine(AppContext.BaseDirectory, "logs");
            Directory.CreateDirectory(logDirectory);
            File.AppendAllText(Path.Combine(logDirectory, "terrain_movement_block.log"), line + Environment.NewLine);
        }
        catch
        {
        }
        ...
        out string reason)
    {
        ...
        if (TryFindFieldCompositeInteractionContact(world, runtimeGrid, footprint, out Vector2 compositeMtv))
        {
    ...
    private IReadOnlyList<FacilityCollisionShape> EnsureFacilityCollisionShapes(RuntimeGridData runtimeGrid)
    {
        ...
        float facilityInsetWorld = ResolveFacilityCollisionInsetWorld();
        foreach (FacilityRegion facility in _facilities)
        {
    ...
    private static bool TryInsetCollisionPolygon(IReadOnlyList<Vector2> polygon, float insetWorld, out Vector2[] insetPolygon)
    {
        ...
        if (polygon.Count < 3 || insetWorld <= 1e-5f)
        {
            return false;
        }
    ...
    {
        return runtimeGrid.TrySampleFacetCollisionSurface(worldX, worldY, out float facetHeight, out _, out _)
