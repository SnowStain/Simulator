...
internal static class ProjectileObstacleResolver
{
        ...
        out ProjectileObstacleHit hit)
    {
        ...
        if (string.Equals(entity.EntityType, "outpost", StringComparison.OrdinalIgnoreCase))
        {
            ...
            TryStoreBest(TryIntersectCylinderDisc(start, end, center + up * (headBaseHeight - 0.07f), up, new Vector3(MathF.Cos((float)(world.GameTimeSec * Math.PI * 0.8)), 0f, MathF.Sin((float)(world.GameTimeSec * Math.PI * 0.8))), towerRadius + 0.055f, 0.030f, projectileRadiusM, entity.EntityType), ref best);
        }
