...
internal sealed partial class Simulator3dForm
{
        ...
        StructureRenderPass renderPass = StructureRenderPass.Full)
    {
        ...
        if (HasFineTerrainOutpostForTeam(entity.Team))
        {
            string? fineLockedPlateId = ResolveLockedPlateIdFor(entity);
            if (TryDrawFineTerrainOutpost(graphics, entity, fineLockedPlateId, renderPass))
            {
        ...
        {
            float ringYaw = (float)(SimulationCombatMath.ResolveOutpostRingYawDeg(entity, _host.World.GameTimeSec) * Math.PI / 180.0);
            DrawCylinderSolid(
                graphics,
        ...
        StructureRenderPass renderPass = StructureRenderPass.Full)
    {
        ...
        float shoulderHeight = Math.Clamp(
            ...
            slabHeight + 0.12f,
            roofHeight - 0.05f);
        if (HasFineTerrainBaseForTeam(entity.Team))
        {
            string? fineLockedPlateId = ResolveLockedPlateIdFor(entity);
        ...
        {
            string? lockedPlateId = ResolveLockedPlateIdFor(entity);
            float slideM = entity.IsAlive ? ResolveBaseTopArmorSlideM(_host.World.GameTimeSec) : 0f;
    ...
    private float DrawEnergyMechanismModel(Graphics graphics, FacilityRegion region, double? overrideCenterWorldX = null, double? overrideCenterWorldY = null)
    {
        ...
        try
        {
            ...
            DrawEnergyMechanismStateOverlay(graphics, centerWorldX, centerWorldY);
        }
        finally
        {
    ...
    private void DrawEnergyMechanismAnnulus(
        Graphics graphics,
        Vector3 center,
        Vector3 normalAxis,
        Vector3 upAxis,
        ...
        int segmentCount)
    {
        float safeInner = Math.Max(0.002f, innerRadius);
        float safeOuter = Math.Max(safeInner + 0.002f, outerRadius);
        int segments = Math.Clamp(segmentCount, 10, 40);
        Vector3 normal = normalAxis.LengthSquared() <= 1e-8f ? Vector3.UnitZ : Vector3.Normalize(normalAxis);
        Vector3 up = upAxis - normal * Vector3.Dot(upAxis, normal);
        if (up.LengthSquared() <= 1e-8f)
        {
            up = Math.Abs(Vector3.Dot(normal, Vector3.UnitY)) >= 0.94f ? Vector3.UnitX : Vector3.UnitY;
        ...
        for (int index = 0; index < segments; index++)
        {
            ...
            float angle1 = MathF.Tau * (index + 1) / segments;
            Vector3 outer0 = center + right * MathF.Cos(angle0) * safeOuter + up * MathF.Sin(angle0) * safeOuter;
            Vector3 outer1 = center + right * MathF.Cos(angle1) * safeOuter + up * MathF.Sin(angle1) * safeOuter;
            Vector3 inner1 = center + right * MathF.Cos(angle1) * safeInner + up * MathF.Sin(angle1) * safeInner;
            Vector3 inner0 = center + right * MathF.Cos(angle0) * safeInner + up * MathF.Sin(angle0) * safeInner;
            outerLoop.Add(outer0);
            innerLoop.Add(inner0);
            Vector3[] quad = { outer0, outer1, inner1, inner0 };
            if (TryBuildProjectedFace(quad, fillColor, edgeColor, out ProjectedFace face))
            {
                faces.Add(face);
            }
        }
    ...
    private void DrawProjectedLoop(Graphics graphics, IReadOnlyList<Vector3> loop, Color color, float width)
    {
        ...
        var points = new List<PointF>(loop.Count + 1);
        for (int index = 0; index < loop.Count; index++)
        {
            if (!TryProject(loop[index], out PointF screenPoint, out _))
            {
                return;
            }
    ...
    private static void ResolveEnergyDiskAxes(ArmorPlateTarget plate, out Vector3 normal, out Vector3 upAxis)
    {
    ...
    private void DrawCpuAnnulusDoubleSided(Graphics graphics, Vector3 center, Vector3 normalAxis, Vector3 upAxis, float innerRadius, float outerRadius, Color color, int segmentCount, float offset)
    {
        Vector3 normal = normalAxis.LengthSquared() <= 1e-8f ? Vector3.UnitX : Vector3.Normalize(normalAxis);
        DrawEnergyMechanismAnnulus(graphics, center + normal * offset, normal, upAxis, innerRadius, outerRadius, color, segmentCount);
        DrawEnergyMechanismAnnulus(graphics, center - normal * offset, -normal, upAxis, innerRadius, outerRadius, color, segmentCount);
    }

    private void DrawCpuMarkerQuad(Graphics graphics, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, Color color)
    {
        ...
        using var fill = new SolidBrush(Color.FromArgb(Math.Min(228, (int)color.A), color));
        using var border = new Pen(Color.FromArgb(Math.Min(250, (int)color.A), BlendColor(color, Color.White, 0.18f)), 1.1f)
        {
            LineJoin = LineJoin.Round,
        };
        graphics.FillPolygon(fill, polygon);
        graphics.DrawPolygon(border, polygon);
    }
    ...
    {
        if (IsHeroDeploymentSubviewOnlyMode())
        {
            return null;
        }
