...
internal static class EnergyMechanismGeometry
{
    ...
    public static Vector3 ResolveRotorHubOffset(
        ...
        string team)
    {
        ...
        float cantileverPairGap = Math.Max(
            frameWidth + cantileverLength,
            profile.StructureCantileverPairGapM > 0f ? profile.StructureCantileverPairGapM : frameWidth + cantileverLength);
        float rotorCenterY = rotorCenterHeight + profile.StructureCantileverOffsetYM;
        ...
        float rotorSide = string.Equals(team, "red", StringComparison.OrdinalIgnoreCase) ? -0.5f : 0.5f;
        return groundRight * (rotorAxisGap * rotorSide) + Vector3.UnitY * rotorCenterY;
    }
